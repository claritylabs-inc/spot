...
export const presentationReferenceSchema = z.object({
  id,
  kind: z.enum(["policy", "requirement", "request", "proposal", "provider", "file", "source"]),
  recordId: id,
  label,
...
export const presentationPropsSchemas = {
  ...
  ComparisonTable: z.object({
    ...
    rows: z.array(z.object({ label, values: z.array(text).max(8), sourceIds }).strict()).min(1).max(40),
  }).strict(),
  RecordList: z.object({ records: z.array(z.object({ referenceId: id, detail: text.optional() }).strict()).min(1).max(40) }).strict(),
  FindingsList: z.object({ findings: z.array(finding).min(1).max(40) }).strict(),
  RequirementMatrix: z.object({ requirements: z.array(z.object({
    label, evidence: text, status: z.enum(["satisfied", "missing", "uncertain"]), sourceIds,
  }).strict()).min(1).max(40) }).strict(),
  DateList: z.object({ dates: z.array(z.object({ label, value: text, sourceIds }).strict()).min(1).max(40) }).strict(),
  SourceReference: z.object({ referenceId: id }).strict(),
  FileReference: z.object({ referenceId: id }).strict(),
  ChoiceGroup: z.object({ label, options: z.array(choice).min(1).max(20), submitLabel: label }).strict(),
  RecordSelector: z.object({ label, referenceIds: z.array(id).min(1).max(40), submitLabel: label }).strict(),
  ClarificationForm: z.object({ fields: z.array(field).min(1).max(8), submitLabel: label }).strict(),
  ActionGroup: z.object({ actions: z.array(z.object({
    label, referenceId: id.optional(), followUp: z.string().min(1).max(1000).optional(),
  }).strict().refine(action => Boolean(action.referenceId) !== Boolean(action.followUp))).min(1).max(6) }).strict(),
} as const;
...
export type PresentationProps<K extends PresentationComponent> = z.infer<(typeof presentationPropsSchemas)[K]>;
export type PresentationElement = {
  [K in PresentationComponent]: { type: K; props: PresentationProps<K>; children: string[] }
}[PresentationComponent];
export type PresentationSpec = { root: string; elements: Record<string, PresentationElement> };
...
export type PresentationCandidate = { id: string; description: string; element: PresentationElement; resource?: string };
export type PresentationToolResult = { name: string; input?: unknown; output: unknown };
...
const envelopeSchema = z.object({
  ...
  spec: z.object({ root: id, elements: z.record(id, z.object({ type: z.string(), props: z.unknown(), children: z.array(id).max(24) }).strict()) }).strict(),
...
export function parseChatPresentation(value: unknown): ChatPresentation | null {
  try {
    if (new TextEncoder().encode(JSON.stringify(value)).length > CHAT_PRESENTATION_MAX_BYTES) return null;
    ...
    for (const reference of references.values()) {
      if (reference.href && (!reference.href.startsWith("/") || reference.href.startsWith("//") || /[\\\s]/.test(reference.href))) return null;
    }
    ...
    for (const [key, element] of entries) {
      ...
      if (element.children.length && type !== "Stack" && type !== "Section") return null;
      if (type === "ComparisonTable") {
        const props = parsed.data as PresentationProps<"ComparisonTable">;
        if (new Set(props.columns.map(column => column.id)).size !== props.columns.length || props.rows.some(row => row.values.length !== props.columns.length)) return null;
      }
      const checkReferences = (data: unknown): boolean => {
        ...
        return Object.entries(data).every(([name, item]) => {
          if (name === "referenceId") return typeof item === "string" && references.has(item);
          if (name === "referenceIds" || name === "sourceIds") return Array.isArray(item) && item.every(key => typeof key === "string" && references.has(key));
      ...
      elements[key] = { type, props: parsed.data, children: element.children } as PresentationElement;
    }
    const visited = new Set<string>();
    const visit = (key: string, depth: number): boolean => {
      if (depth > 4 || visited.has(key) || !Object.hasOwn(elements, key)) return false;
      visited.add(key);
      return elements[key].children.every(child => visit(child, depth + 1));
    };
    if (!visit(envelope.spec.root, 0) || visited.size !== entries.length) return null;
