...
Update confirmations list supplied field values, including explicit clears and replacement lists. Omitted update fields remain unchanged; null is accepted only on fields whose schema explicitly describes a deliberate clear. Broker profile approvals show saved → proposed values with readable coverage names and optional `evidence` (source URLs or mailbox/sender/date) bound to the exact input; evidence is approval context, not a broker-profile field. The operator runner stops at its first pending confirmation, blocks later calls in that batch, checkpoints research for resumption, and ignores stale segment completion/failure callbacks after approval resumes the task. Invalid schema and confirmation-preflight results return a structured `failure` with phase, code, recovery guidance, and authoritative `writeState: not_started`, so the model may correct the call in the same task.
