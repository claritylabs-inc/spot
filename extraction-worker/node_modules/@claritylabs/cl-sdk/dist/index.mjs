...
var ModelGenerationFailure = class extends Error {
  constructor(options) {
...
// src/schemas/acord-taxonomy.generated.ts
var ACORD_TAXONOMY_METADATA = {
  "sources": {
...
var ACORD_LOB_SOURCE_HEADERS = [
  "Value",
  "Description"
];
var ACORD_LOB_SOURCE_ROWS = [
  [
    "ACHE",
    "Accident and Health"
  ],
...
var ACORD_LOB_ENTRIES = ACORD_LOB_SOURCE_ROWS.map(([code, description]) => ({ code, description }));
var ACORD_LOB_CODES = [
  "ACHE",
  "ACCT",
  "AGOP",
...
var ACORD_COVERAGE_SOURCE_HEADERS = [
  "Value",
  "Description"
];
var ACORD_COVERAGE_SOURCE_ROWS = [
  [
    "X12M",
...
var ACORD_COVERAGE_ENTRIES = ACORD_COVERAGE_SOURCE_ROWS.map(([code, description]) => ({ code, description }));
var ACORD_COVERAGE_CODES = [
  "X12M",
  "24HR",
  "TFSCC",
  "AOP5",
...
var ACORD_COVERAGE_DESCRIPTIONS = {
  "X12M": [
    "12 Month Extension Clause"
  ],
  "24HR": [
...
var LEGACY_ACORD_LOB_CODE_TO_CURRENT = {
  AAPPL: "AIRC",
  APKG: "APKGE",
  ARVP: "RECV",
  CEQFL: "INMRC",
...
function toLobCodes(values) {
  ...
  return normalized.some((code) => code !== "UN") ? normalized.filter((code) => code !== "UN") : ["UN"];
}
...
// src/schemas/coverage-codes.ts
import { z as z4 } from "zod";
var COVERAGE_CODE_SET = new Set(ACORD_COVERAGE_CODES);
var COVERAGE_DESCRIPTIONS = ACORD_COVERAGE_DESCRIPTIONS;
function normalizedDescription(value) {
  return value.trim().toLowerCase().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();
}
var COVERAGE_CODES_BY_DESCRIPTION = /* @__PURE__ */ new Map();
for (const code of ACORD_COVERAGE_CODES) {
  const coverageCode = code;
...
function isAcordCoverageCode(value) {
  return typeof value === "string" && COVERAGE_CODE_SET.has(value);
}
function normalizeAcordCoverageCode(value) {
  if (typeof value !== "string") return void 0;
  const code = value.trim().toUpperCase();
  return isAcordCoverageCode(code) ? code : void 0;
}
var AcordCoverageCodeSchema = z4.string().transform(
  (value, context) => {
...
function coverageDescriptions(value) {
  const code = normalizeAcordCoverageCode(value);
...
function coverageCodesForDescription(value) {
  if (typeof value !== "string") return [];
  return COVERAGE_CODES_BY_DESCRIPTION.get(normalizedDescription(value)) ?? [];
}
function resolveAcordCoverageCode(code, sourceDescription) {
...
var SourceProvenanceSchema = z5.object({
  sourceSpanIds: z5.array(z5.string().min(1)).min(1),
  documentNodeId: z5.string().optional(),
  sourceTextHash: z5.string().optional(),
  pageStart: z5.number().int().positive().optional(),
  pageEnd: z5.number().int().positive().optional()
});
var AddressSchema = z5.object({
  street1: z5.string(),
  street2: z5.string().optional(),
  city: z5.string(),
  state: z5.string(),
  zip: z5.string(),
...
var ContactSchema = z5.object({
  ...
  fax: z5.string().optional(),
  email: z5.string().optional(),
  address: AddressSchema.optional(),
  hours: z5.string().optional()
}).merge(SourceProvenanceSchema);
var FormReferenceSchema = z5.object({
  formNumber: z5.string(),
  editionDate: z5.string().optional(),
  title: z5.string().optional(),
  formType: z5.enum(["coverage", "endorsement", "declarations", "application", "notice", "other"]),
  pageStart: z5.number().optional(),
  pageEnd: z5.number().optional(),
  documentNodeId: z5.string().optional(),
  sourceSpanIds: z5.array(z5.string()).optional(),
  sourceTextHash: z5.string().optional()
});
var TaxFeeItemSchema = z5.object({
  name: z5.string(),
  amount: z5.string(),
  amountValue: z5.number().optional(),
  type: z5.enum(["tax", "fee", "surcharge", "assessment"]).optional(),
  description: z5.string().optional(),
  documentNodeId: z5.string().optional(),
  sourceSpanIds: z5.array(z5.string()).optional(),
  sourceTextHash: z5.string().optional()
});
var RatingBasisSchema = z5.object({
  type: RatingBasisTypeSchema,
  amount: z5.string().optional(),
  description: z5.string().optional()
});
var SublimitSchema = z5.object({
  name: z5.string(),
  limit: z5.string(),
  appliesTo: z5.string().optional(),
  deductible: z5.string().optional()
});
var SharedLimitSchema = z5.object({
  description: z5.string(),
  limit: z5.string(),
  coverageParts: z5.array(z5.string())
});
var ExtendedReportingPeriodSchema = z5.object({
  basicDays: z5.number().optional(),
  supplementalYears: z5.number().optional(),
  supplementalPremium: z5.string().optional()
});
var NamedInsuredSchema = z5.object({
  name: z5.string(),
...
var CoverageValueTypeSchema = z6.enum([
  "numeric",
  "included",
  "not_included",
  "as_stated",
...
var EnrichedCoverageSchema = z6.object({
  name: z6.string(),
  coverageCode: z6.string().optional(),
  formNumber: z6.string().optional(),
  formEditionDate: z6.string().optional(),
  limit: z6.string(),
  limitAmount: z6.number().optional(),
  limitType: LimitTypeSchema.optional(),
  limitValueType: CoverageValueTypeSchema.optional(),
  ...
  deductibleValueType: CoverageValueTypeSchema.optional(),
  sir: z6.string().optional(),
  sublimit: z6.string().optional(),
  coinsurance: z6.string().optional(),
  valuation: ValuationMethodSchema.optional(),
  territory: z6.string().optional(),
  trigger: CoverageTriggerSchema.optional(),
  retroactiveDate: z6.string().optional(),
  included: z6.boolean(),
  premium: z6.string().optional(),
  pageNumber: z6.number().optional(),
  sectionRef: z6.string().optional(),
  originalContent: z6.string().optional(),
  recordId: z6.string().optional(),
...
var EndorsementPartySchema = z7.object({
  name: z7.string(),
  role: EndorsementPartyRoleSchema,
  address: AddressSchema.optional(),
  relationship: z7.string().optional(),
  scope: z7.string().optional()
}).merge(SourceProvenanceSchema);
var EndorsementSchema = z7.object({
  formNumber: z7.string(),
  editionDate: z7.string().optional(),
  title: z7.string(),
  endorsementType: EndorsementTypeSchema,
  effectiveDate: z7.string().optional(),
  affectedCoverageParts: z7.array(z7.string()).optional(),
  namedParties: z7.array(EndorsementPartySchema).optional(),
  keyTerms: z7.array(z7.string()).optional(),
  premiumImpact: z7.string().optional(),
  excerpt: z7.string().optional(),
  content: z7.string().optional(),
  pageStart: z7.number(),
  pageEnd: z7.number().optional(),
...
var ExclusionSchema = z8.object({
  name: z8.string(),
  formNumber: z8.string().optional(),
  excludedPerils: z8.array(z8.string()).optional(),
  isAbsolute: z8.boolean().optional(),
  exceptions: z8.array(z8.string()).optional(),
  buybackAvailable: z8.boolean().optional(),
  buybackEndorsement: z8.string().optional(),
  appliesTo: z8.array(z8.string()).optional(),
  content: z8.string(),
...
var ConditionKeyValueSchema = z9.object({
  key: z9.string(),
  value: z9.string()
});
var PolicyConditionSchema = z9.object({
  name: z9.string(),
  conditionType: ConditionTypeSchema,
  content: z9.string(),
  keyValues: z9.array(ConditionKeyValueSchema).optional(),
  pageNumber: z9.number().optional(),
  recordId: z9.string().optional(),
  documentNodeId: z9.string().optional(),
  sourceSpanIds: z9.array(z9.string()).optional(),
  sourceTextHash: z9.string().optional()
});

// src/schemas/parties.ts
import { z as z10 } from "zod";
var InsurerInfoSchema = z10.object({
  legalName: z10.string(),
  naicNumber: z10.string().optional(),
  amBestRating: z10.string().optional(),
  amBestNumber: z10.string().optional(),
  admittedStatus: AdmittedStatusSchema.optional(),
  stateOfDomicile: z10.string().optional(),
  address: AddressSchema.optional()
}).merge(SourceProvenanceSchema);
var ProducerInfoSchema = z10.object({
  agencyName: z10.string(),
  contactName: z10.string().optional(),
...
var PaymentInstallmentSchema = z11.object({
  dueDate: z11.string(),
  amount: z11.string(),
  description: z11.string().optional()
});
var PaymentPlanSchema = z11.object({
  installments: z11.array(PaymentInstallmentSchema),
  financeCharge: z11.string().optional()
});
var LocationPremiumSchema = z11.object({
  locationNumber: z11.number(),
  premium: z11.string(),
  description: z11.string().optional()
});
...
var ClaimRecordSchema = z12.object({
  dateOfLoss: z12.string(),
  claimNumber: z12.string().optional(),
  description: z12.string(),
  status: ClaimStatusSchema,
  paid: z12.string().optional(),
  reserved: z12.string().optional(),
  incurred: z12.string().optional(),
  claimant: z12.string().optional(),
  coverageLine: z12.string().optional()
});
var LossSummarySchema = z12.object({
  period: z12.string().optional(),
  totalClaims: z12.number().optional(),
  totalIncurred: z12.string().optional(),
  totalPaid: z12.string().optional(),
  totalReserved: z12.string().optional(),
  lossRatio: z12.string().optional()
});
var ExperienceModSchema = z12.object({
  factor: z12.number(),
  effectiveDate: z12.string().optional(),
  state: z12.string().optional()
});

// src/schemas/underwriting.ts
import { z as z13 } from "zod";
var EnrichedSubjectivitySchema = z13.object({
  description: z13.string(),
  category: SubjectivityCategorySchema.optional(),
  dueDate: z13.string().optional(),
  status: z13.enum(["open", "satisfied", "waived"]).optional(),
  pageNumber: z13.number().optional()
});
var EnrichedUnderwritingConditionSchema = z13.object({
  description: z13.string(),
  category: z13.string().optional(),
  pageNumber: z13.number().optional()
});
var BindingAuthoritySchema = z13.object({
  authorizedBy: z13.string().optional(),
  method: z13.string().optional(),
  expiration: z13.string().optional(),
  conditions: z13.array(z13.string()).optional()
});
...
var EmployersLiabilityLimitsSchema = z14.object({
  eachAccident: z14.string(),
  diseasePolicyLimit: z14.string(),
  diseaseEachEmployee: z14.string()
});
var LimitScheduleSchema = z14.object({
  perOccurrence: z14.string().optional(),
  ...
  eachEmployee: z14.string().optional(),
  ...
  eachOccurrenceUmbrella: z14.string().optional(),
  umbrellaAggregate: z14.string().optional(),
  umbrellaRetention: z14.string().optional(),
  statutory: z14.boolean().optional(),
  employersLiability: EmployersLiabilityLimitsSchema.optional(),
  sublimits: z14.array(SublimitSchema).optional(),
  sharedLimits: z14.array(SharedLimitSchema).optional(),
  defenseCostTreatment: DefenseCostTreatmentSchema.optional()
});
var DeductibleScheduleSchema = z14.object({
  perClaim: z14.string().optional(),
  perOccurrence: z14.string().optional(),
  aggregateDeductible: z14.string().optional(),
  selfInsuredRetention: z14.string().optional(),
  corridorDeductible: z14.string().optional(),
  waitingPeriod: z14.string().optional(),
  appliesTo: z14.enum(["damages_only", "damages_and_defense", "defense_only"]).optional()
});
var InsuredLocationSchema = z14.object({
  number: z14.number(),
  address: AddressSchema,
  description: z14.string().optional(),
  buildingValue: z14.string().optional(),
  contentsValue: z14.string().optional(),
  businessIncomeValue: z14.string().optional(),
  constructionType: z14.string().optional(),
  yearBuilt: z14.number().optional(),
  squareFootage: z14.number().optional(),
  protectionClass: z14.string().optional(),
  sprinklered: z14.boolean().optional(),
  alarmType: z14.string().optional(),
  occupancy: z14.string().optional()
});
var VehicleCoverageSchema = z14.object({
  type: VehicleCoverageTypeSchema,
  limit: z14.string().optional(),
  deductible: z14.string().optional(),
  included: z14.boolean()
});
var InsuredVehicleSchema = z14.object({
  number: z14.number(),
  year: z14.number(),
  make: z14.string(),
  model: z14.string(),
  vin: z14.string(),
  costNew: z14.string().optional(),
  statedValue: z14.string().optional(),
  garageLocation: z14.number().optional(),
  coverages: z14.array(VehicleCoverageSchema).optional(),
  radius: z14.string().optional(),
  vehicleType: z14.string().optional()
});
var ClassificationCodeSchema = z14.object({
  code: z14.string(),
  description: z14.string(),
  premiumBasis: z14.string(),
  basisAmount: z14.string().optional(),
  rate: z14.string().optional(),
  premium: z14.string().optional(),
  locationNumber: z14.number().optional()
});
var DwellingDetailsSchema = z14.object({
  constructionType: ConstructionTypeSchema.optional(),
  yearBuilt: z14.number().optional(),
  squareFootage: z14.number().optional(),
  stories: z14.number().optional(),
  roofType: RoofTypeSchema.optional(),
  roofAge: z14.number().optional(),
  heatingType: z14.enum(["central", "baseboard", "radiant", "space_heater", "heat_pump", "other"]).optional(),
  foundationType: FoundationTypeSchema.optional(),
  plumbingType: z14.enum(["copper", "pex", "galvanized", "polybutylene", "cpvc", "other"]).optional(),
  electricalType: z14.enum(["circuit_breaker", "fuse_box", "knob_and_tube", "other"]).optional(),
  electricalAmps: z14.number().optional(),
  hasSwimmingPool: z14.boolean().optional(),
  poolType: z14.enum(["in_ground", "above_ground"]).optional(),
  hasTrampoline: z14.boolean().optional(),
  hasDog: z14.boolean().optional(),
  dogBreed: z14.string().optional(),
  protectiveDevices: z14.array(z14.string()).optional(),
  distanceToFireStation: z14.string().optional(),
  distanceToHydrant: z14.string().optional(),
  fireProtectionClass: z14.string().optional()
});
var DriverRecordSchema = z14.object({
  name: z14.string(),
  dateOfBirth: z14.string().optional(),
  licenseNumber: z14.string().optional(),
  licenseState: z14.string().optional(),
  relationship: z14.enum(["named_insured", "spouse", "child", "other_household", "other"]).optional(),
  yearsLicensed: z14.number().optional(),
  gender: z14.string().optional(),
  maritalStatus: z14.string().optional(),
  goodStudentDiscount: z14.boolean().optional(),
  defensiveDriverDiscount: z14.boolean().optional(),
  violations: z14.array(z14.object({
    date: z14.string().optional(),
    type: z14.string().optional(),
    description: z14.string().optional()
  })).optional(),
  accidents: z14.array(z14.object({
    date: z14.string().optional(),
    atFault: z14.boolean().optional(),
    description: z14.string().optional(),
    amountPaid: z14.string().optional()
  })).optional(),
  sr22Required: z14.boolean().optional()
});
var PersonalVehicleDetailsSchema = z14.object({
  number: z14.number().optional(),
  year: z14.number().optional(),
  make: z14.string().optional(),
  model: z14.string().optional(),
  vin: z14.string().optional(),
  bodyType: z14.string().optional(),
  garagingAddress: AddressSchema.optional(),
  usage: PersonalAutoUsageSchema.optional(),
  annualMileage: z14.number().optional(),
  odometerReading: z14.number().optional(),
  driverAssignment: z14.string().optional(),
  ...
  rentalReimbursement: z14.boolean().optional(),
  towing: z14.boolean().optional()
});

// src/schemas/declarations/personal.ts
var HomeownersDeclarationsSchema = z15.object({
  line: z15.literal("homeowners"),
  formType: HomeownersFormTypeSchema,
  coverageA: z15.string().optional(),
  coverageB: z15.string().optional(),
  coverageC: z15.string().optional(),
  coverageD: z15.string().optional(),
  coverageE: z15.string().optional(),
  coverageF: z15.string().optional(),
  allPerilDeductible: z15.string().optional(),
  windHailDeductible: z15.string().optional(),
  hurricaneDeductible: z15.string().optional(),
  lossSettlement: LossSettlementSchema.optional(),
  dwelling: DwellingDetailsSchema,
  mortgagee: EndorsementPartySchema.optional(),
  additionalMortgagees: z15.array(EndorsementPartySchema).optional()
});
var PersonalAutoDeclarationsSchema = z15.object({
  line: z15.literal("personal_auto"),
  vehicles: z15.array(PersonalVehicleDetailsSchema),
  drivers: z15.array(DriverRecordSchema),
  liabilityLimits: z15.object({
    bodilyInjuryPerPerson: z15.string().optional(),
    bodilyInjuryPerAccident: z15.string().optional(),
    propertyDamage: z15.string().optional(),
    combinedSingleLimit: z15.string().optional()
  }).optional(),
  umLimits: z15.object({
    bodilyInjuryPerPerson: z15.string().optional(),
    bodilyInjuryPerAccident: z15.string().optional()
  }).optional(),
  uimLimits: z15.object({
    bodilyInjuryPerPerson: z15.string().optional(),
    bodilyInjuryPerAccident: z15.string().optional()
  }).optional(),
  pipLimit: z15.string().optional(),
  medPayLimit: z15.string().optional()
});
var DwellingFireDeclarationsSchema = z15.object({
  line: z15.literal("dwelling_fire"),
  formType: DwellingFireFormTypeSchema,
  dwellingLimit: z15.string().optional(),
  otherStructuresLimit: z15.string().optional(),
  personalPropertyLimit: z15.string().optional(),
  fairRentalValueLimit: z15.string().optional(),
  liabilityLimit: z15.string().optional(),
  medicalPaymentsLimit: z15.string().optional(),
  deductible: z15.string().optional(),
  dwelling: DwellingDetailsSchema
});
var FloodDeclarationsSchema = z15.object({
  line: z15.literal("flood"),
  programType: z15.enum(["nfip", "private"]),
  floodZone: FloodZoneSchema.optional(),
  communityNumber: z15.string().optional(),
  communityRating: z15.number().optional(),
  buildingCoverage: z15.string().optional(),
  contentsCoverage: z15.string().optional(),
  iccCoverage: z15.string().optional(),
  deductible: z15.string().optional(),
  waitingPeriodDays: z15.number().optional(),
  elevationCertificate: z15.boolean().optional(),
  elevationDifference: z15.string().optional(),
  buildingDiagramNumber: z15.number().optional(),
  basementOrEnclosure: z15.boolean().optional(),
  postFirmConstruction: z15.boolean().optional()
});
var EarthquakeDeclarationsSchema = z15.object({
  line: z15.literal("earthquake"),
  dwellingCoverage: z15.string().optional(),
  contentsCoverage: z15.string().optional(),
  lossOfUseCoverage: z15.string().optional(),
  deductiblePercent: z15.number().optional(),
  retrofitDiscount: z15.boolean().optional(),
  masonryVeneerCoverage: z15.boolean().optional()
});
var PersonalUmbrellaDeclarationsSchema = z15.object({
  line: z15.literal("personal_umbrella"),
  perOccurrenceLimit: z15.string().optional(),
  aggregateLimit: z15.string().optional(),
  retainedLimit: z15.string().optional(),
...
var PersonalArticlesDeclarationsSchema = z15.object({
  line: z15.literal("personal_articles"),
  scheduledItems: z15.array(z15.object({
    itemNumber: z15.number().optional(),
    category: ScheduledItemCategorySchema.optional(),
    description: z15.string(),
    appraisedValue: z15.string(),
    appraisalDate: z15.string().optional()
  })),
  blanketCoverage: z15.string().optional(),
  deductible: z15.string().optional(),
  worldwideCoverage: z15.boolean().optional(),
  breakageCoverage: z15.boolean().optional()
});
var WatercraftDeclarationsSchema = z15.object({
  line: z15.literal("watercraft"),
  boatType: BoatTypeSchema.optional(),
  year: z15.number().optional(),
  make: z15.string().optional(),
  model: z15.string().optional(),
  length: z15.string().optional(),
  hullMaterial: z15.enum(["fiberglass", "aluminum", "wood", "steel", "inflatable", "other"]).optional(),
  hullValue: z15.string().optional(),
  motorHorsepower: z15.number().optional(),
  motorType: z15.enum(["outboard", "inboard", "inboard_outboard", "jet"]).optional(),
  navigationLimits: z15.string().optional(),
  layupPeriod: z15.string().optional(),
  liabilityLimit: z15.string().optional(),
  medicalPaymentsLimit: z15.string().optional(),
  physicalDamageDeductible: z15.string().optional(),
  uninsuredBoaterLimit: z15.string().optional(),
  trailerCovered: z15.boolean().optional(),
  trailerValue: z15.string().optional()
});
var RecreationalVehicleDeclarationsSchema = z15.object({
  line: z15.literal("recreational_vehicle"),
  vehicleType: RVTypeSchema,
  year: z15.number().optional(),
  make: z15.string().optional(),
  model: z15.string().optional(),
  vin: z15.string().optional(),
  value: z15.string().optional(),
  liabilityLimit: z15.string().optional(),
  collisionDeductible: z15.string().optional(),
  comprehensiveDeductible: z15.string().optional(),
  personalEffectsCoverage: z15.string().optional(),
  fullTimerCoverage: z15.boolean().optional()
});
var FarmRanchDeclarationsSchema = z15.object({
  line: z15.literal("farm_ranch"),
  dwellingCoverage: z15.string().optional(),
  farmPersonalPropertyCoverage: z15.string().optional(),
  farmLiabilityLimit: z15.string().optional(),
  farmAutoIncluded: z15.boolean().optional(),
  livestock: z15.array(z15.object({
    type: z15.string(),
    headCount: z15.number(),
    value: z15.string().optional()
  })).optional(),
  equipmentSchedule: z15.array(z15.object({
    description: z15.string(),
    value: z15.string()
  })).optional(),
  acreage: z15.number().optional(),
  dwelling: DwellingDetailsSchema.optional()
});
var TitleDeclarationsSchema = z15.object({
  line: z15.literal("title"),
  policyType: TitlePolicyTypeSchema,
  policyAmount: z15.string(),
  legalDescription: z15.string().optional(),
  propertyAddress: AddressSchema.optional(),
  effectiveDate: z15.string().optional(),
  exceptions: z15.array(z15.object({
    number: z15.number(),
    description: z15.string()
  })).optional(),
  underwriter: z15.string().optional()
});
var PetDeclarationsSchema = z15.object({
  line: z15.literal("pet"),
  species: PetSpeciesSchema,
  breed: z15.string().optional(),
  petName: z15.string().optional(),
  age: z15.number().optional(),
  annualLimit: z15.string().optional(),
  perIncidentLimit: z15.string().optional(),
  deductible: z15.string().optional(),
  reimbursementPercent: z15.number().optional(),
  waitingPeriodDays: z15.number().optional(),
  preExistingConditionsExcluded: z15.boolean().optional(),
  wellnessCoverage: z15.boolean().optional()
});
var TravelDeclarationsSchema = z15.object({
  line: z15.literal("travel"),
  tripDepartureDate: z15.string().optional(),
  tripReturnDate: z15.string().optional(),
  destinations: z15.array(z15.string()).optional(),
  travelers: z15.array(z15.object({
    name: z15.string(),
    age: z15.number().optional()
  })).optional(),
  tripCost: z15.string().optional(),
  tripCancellationLimit: z15.string().optional(),
  medicalLimit: z15.string().optional(),
  evacuationLimit: z15.string().optional(),
  baggageLimit: z15.string().optional()
});
var IdentityTheftDeclarationsSchema = z15.object({
  line: z15.literal("identity_theft"),
  coverageLimit: z15.string().optional(),
  expenseReimbursement: z15.string().optional(),
  creditMonitoring: z15.boolean().optional(),
  restorationServices: z15.boolean().optional(),
  lostWagesLimit: z15.string().optional()
});

// src/schemas/declarations/commercial.ts
import { z as z16 } from "zod";
var GLDeclarationsSchema = z16.object({
  line: z16.literal("gl"),
  coverageForm: CoverageFormSchema.optional(),
  perOccurrenceLimit: z16.string().optional(),
  generalAggregate: z16.string().optional(),
  productsCompletedOpsAggregate: z16.string().optional(),
  personalAdvertisingInjury: z16.string().optional(),
  fireDamage: z16.string().optional(),
  medicalExpense: z16.string().optional(),
  defenseCostTreatment: DefenseCostTreatmentSchema.optional(),
  deductible: z16.string().optional(),
  classifications: z16.array(ClassificationCodeSchema).optional(),
  retroactiveDate: z16.string().optional()
});
var CommercialPropertyDeclarationsSchema = z16.object({
  line: z16.literal("commercial_property"),
  causesOfLossForm: z16.enum(["basic", "broad", "special"]).optional(),
  coinsurancePercent: z16.number().optional(),
  valuationMethod: ValuationMethodSchema.optional(),
  locations: z16.array(InsuredLocationSchema),
  blanketLimit: z16.string().optional(),
  businessIncomeLimit: z16.string().optional(),
  extraExpenseLimit: z16.string().optional()
});
var CommercialAutoDeclarationsSchema = z16.object({
  line: z16.literal("commercial_auto"),
  vehicles: z16.array(InsuredVehicleSchema),
  coveredAutoSymbols: z16.array(z16.number()).optional(),
  liabilityLimit: z16.string().optional(),
  umLimit: z16.string().optional(),
  uimLimit: z16.string().optional(),
  hiredAutoLiability: z16.boolean().optional(),
  nonOwnedAutoLiability: z16.boolean().optional()
});
var WorkersCompDeclarationsSchema = z16.object({
  line: z16.literal("workers_comp"),
  coveredStates: z16.array(z16.string()).optional(),
  classifications: z16.array(ClassificationCodeSchema),
  experienceMod: ExperienceModSchema.optional(),
  employersLiability: EmployersLiabilityLimitsSchema.optional()
});
var UmbrellaExcessDeclarationsSchema = z16.object({
  line: z16.literal("umbrella_excess"),
  perOccurrenceLimit: z16.string().optional(),
  aggregateLimit: z16.string().optional(),
  retention: z16.string().optional(),
  underlyingPolicies: z16.array(z16.object({
    carrier: z16.string().optional(),
    policyNumber: z16.string().optional(),
    policyType: z16.string().optional(),
    limits: z16.string().optional()
  }))
});
var ProfessionalLiabilityDeclarationsSchema = z16.object({
  line: z16.literal("professional_liability"),
  perClaimLimit: z16.string().optional(),
  aggregateLimit: z16.string().optional(),
  retroactiveDate: z16.string().optional(),
  defenseCostTreatment: DefenseCostTreatmentSchema.optional(),
  extendedReportingPeriod: ExtendedReportingPeriodSchema.optional()
});
var CyberDeclarationsSchema = z16.object({
  line: z16.literal("cyber"),
  aggregateLimit: z16.string().optional(),
  retroactiveDate: z16.string().optional(),
  waitingPeriodHours: z16.number().optional(),
  sublimits: z16.array(z16.object({
    coverageName: z16.string(),
    limit: z16.string()
  })).optional()
});
var DODeclarationsSchema = z16.object({
  line: z16.literal("directors_officers"),
  sideALimit: z16.string().optional(),
  sideBLimit: z16.string().optional(),
  sideCLimit: z16.string().optional(),
  sideARetention: z16.string().optional(),
  sideBRetention: z16.string().optional(),
  sideCRetention: z16.string().optional(),
  continuityDate: z16.string().optional()
});
var CrimeDeclarationsSchema = z16.object({
  line: z16.literal("crime"),
  formType: z16.enum(["discovery", "loss_sustained"]).optional(),
  agreements: z16.array(z16.object({
    agreement: z16.string(),
    coverageName: z16.string(),
    limit: z16.string(),
    deductible: z16.string()
  }))
});

// src/schemas/declarations/index.ts
var DeclarationsSchema = z17.discriminatedUnion("line", [
  // Personal lines
...
var SourceSpanKindSchema = z18.enum([
  "pdf_text",
  "pdf_image",
  "html",
  "markdown",
...
var SourceSpanUnitSchema = z18.enum([
  "page",
  "section",
  "table",
  "table_row",
...
var SourceSpanBBoxSchema = z18.object({
...
var SourceSpanLocationSchema = z18.object({
  page: z18.number().int().positive().optional(),
  startPage: z18.number().int().positive().optional(),
  endPage: z18.number().int().positive().optional(),
  charStart: z18.number().int().nonnegative().optional(),
  charEnd: z18.number().int().nonnegative().optional(),
  lineStart: z18.number().int().positive().optional(),
  lineEnd: z18.number().int().positive().optional(),
  fieldPath: z18.string().optional()
});
var SourceSpanTableLocationSchema = z18.object({
  tableId: z18.string().optional(),
  rowIndex: z18.number().int().nonnegative().optional(),
  columnIndex: z18.number().int().nonnegative().optional(),
  columnName: z18.string().optional(),
  rowSpanId: z18.string().optional(),
  tableSpanId: z18.string().optional(),
  isHeader: z18.boolean().optional()
});
var SourceSpanSchema = z18.object({
  id: z18.string().min(1),
  documentId: z18.string().min(1),
  sourceKind: SourceKindSchema.optional(),
  chunkId: z18.string().optional(),
  kind: SourceSpanKindSchema,
  text: z18.string(),
  hash: z18.string().min(1),
  textHash: z18.string().optional(),
  pageStart: z18.number().int().positive().optional(),
  pageEnd: z18.number().int().positive().optional(),
  sectionId: z18.string().optional(),
  formNumber: z18.string().optional(),
  sourceUnit: SourceSpanUnitSchema.optional(),
  parentSpanId: z18.string().optional(),
  table: SourceSpanTableLocationSchema.optional(),
  bbox: z18.array(SourceSpanBBoxSchema).optional(),
  location: SourceSpanLocationSchema.optional(),
  metadata: z18.record(z18.string(), z18.string()).optional()
});
var SourceSpanRefSchema = z18.object({
  sourceSpanId: z18.string().min(1),
  documentId: z18.string().min(1).optional(),
  chunkId: z18.string().optional(),
  quote: z18.string().optional(),
  hash: z18.string().optional(),
  location: SourceSpanLocationSchema.optional()
});
var SourceChunkSchema = z18.object({
  id: z18.string().min(1),
  documentId: z18.string().min(1),
  sourceSpanIds: z18.array(z18.string().min(1)),
  text: z18.string(),
  textHash: z18.string().min(1),
  pageStart: z18.number().int().positive().optional(),
  pageEnd: z18.number().int().positive().optional(),
  metadata: z18.record(z18.string(), z18.string()).default({})
});
var DocumentSourceNodeKindSchema = z18.enum([
  "document",
  "page_group",
  "page",
  "form",
...
var DocumentSourceNodeSchema = z18.object({
  id: z18.string().min(1),
  documentId: z18.string().min(1),
  parentId: z18.string().optional(),
  kind: DocumentSourceNodeKindSchema,
  title: z18.string(),
  description: z18.string(),
  textExcerpt: z18.string().optional(),
  sourceSpanIds: z18.array(z18.string().min(1)),
  pageStart: z18.number().int().positive().optional(),
  pageEnd: z18.number().int().positive().optional(),
  bbox: z18.array(SourceSpanBBoxSchema).optional(),
...
var SourceBackedValueSchema = z18.object({
  value: z18.string(),
  normalizedValue: z18.string().optional(),
  confidence: z18.enum(["low", "medium", "high"]).default("medium"),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
var OperationalProductIdentitySchema = z18.object({
...
var OperationalAddressSchema = z18.object({
  street1: z18.string().optional(),
  street2: z18.string().optional(),
  city: z18.string().optional(),
  state: z18.string().optional(),
  zip: z18.string().optional(),
  country: z18.string().optional(),
  formatted: z18.string().optional()
});
var OperationalDeclarationFactSchema = z18.object({
  field: z18.enum([
    "namedInsured",
    "mailingAddress",
    "dba",
  ...
  value: z18.string(),
  normalizedValue: z18.string().optional(),
  valueKind: z18.enum(["string", "number", "date", "money", "address", "list", "unknown"]).default("string"),
  address: OperationalAddressSchema.optional(),
  confidence: z18.enum(["low", "medium", "high"]).default("medium"),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
var OperationalCoverageTermSchema = z18.object({
  kind: z18.enum([
    "each_claim_limit",
  ...
  label: z18.string(),
  value: z18.string(),
  amount: z18.number().optional(),
  appliesTo: z18.string().optional(),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
var OperationalCoverageLineSchema = z18.object({
  name: z18.string(),
  lineOfBusiness: AcordLobCodeSchema.optional(),
  coverageCode: z18.string().optional(),
  limit: z18.string().optional(),
  deductible: z18.string().optional(),
  premium: z18.string().optional(),
  retroactiveDate: z18.string().optional(),
  formNumber: z18.string().optional(),
  sectionRef: z18.string().optional(),
  endorsementNumber: z18.string().optional(),
  limits: z18.array(OperationalCoverageTermSchema).default([]),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
var OperationalCoverageScheduleValueSchema = z18.object({
  label: z18.string(),
  value: z18.string()
});
var OperationalCoverageScheduleItemSchema = z18.object({
  label: z18.string(),
  description: z18.string().optional(),
  values: z18.array(OperationalCoverageScheduleValueSchema).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
var OperationalCoverageScheduleSchema = z18.object({
  name: z18.string(),
  kind: z18.enum(["vehicle", "property", "location", "other"]),
  description: z18.string().optional(),
  items: z18.array(OperationalCoverageScheduleItemSchema).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([]),
  pageStart: z18.number().int().positive().optional(),
  pageEnd: z18.number().int().positive().optional()
});
var OperationalPremiumLineSchema = z18.object({
  line: z18.string(),
  amount: z18.string(),
...
var OperationalTaxFeeItemSchema = z18.object({
  name: z18.string(),
  amount: z18.string(),
  amountValue: z18.number().optional(),
...
var OperationalPartySchema = z18.object({
  role: z18.string(),
  name: z18.string(),
  address: OperationalAddressSchema.optional(),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
...
var OperationalEndorsementSupportSchema = z18.object({
  kind: z18.string(),
  status: z18.enum(["supported", "excluded", "requires_review"]),
  summary: z18.string(),
  sourceNodeIds: z18.array(z18.string().min(1)).default([]),
  sourceSpanIds: z18.array(z18.string().min(1)).default([])
});
...
var OperationalLinesOfBusinessSchema = z18.preprocess(
  (value) => Array.isArray(value) ? toLobCodes(value.filter((item) => typeof item === "string")) : void 0,
  z18.array(AcordLobCodeSchema).default(["UN"])
);
var PolicyOperationalProfileSchema = z18.preprocess(
  legacyOperationalProfileInput,
  z18.object({
    documentType: z18.enum(["policy", "quote"]).default("policy"),
    ...
    declarationFacts: z18.array(OperationalDeclarationFactSchema).default([]),
    coverages: z18.array(OperationalCoverageLineSchema).default([]),
    coverageSchedules: z18.array(OperationalCoverageScheduleSchema).optional(),
    premiumBreakdown: z18.array(OperationalPremiumLineSchema).optional(),
    taxesAndFees: z18.array(OperationalTaxFeeItemSchema).optional(),
    totalCost: SourceBackedValueSchema.optional(),
    parties: z18.array(OperationalPartySchema).default([]),
    endorsementSupport: z18.array(OperationalEndorsementSupportSchema).default([]),
    sourceNodeIds: z18.array(z18.string().min(1)).default([]),
    sourceSpanIds: z18.array(z18.string().min(1)).default([]),
    warnings: z18.array(z18.string()).default([])
  })
);

// src/schemas/document.ts
var SubsectionSchema = z19.object({
  title: z19.string(),
  sectionNumber: z19.string().optional(),
  pageNumber: z19.number().optional(),
  excerpt: z19.string().optional(),
  content: z19.string().optional(),
  documentNodeId: z19.string().optional(),
  sourceSpanIds: z19.array(z19.string()).optional(),
  sourceTextHash: z19.string().optional()
});
var SectionSchema = z19.object({
  title: z19.string(),
  sectionNumber: z19.string().optional(),
  pageStart: z19.number(),
  pageEnd: z19.number().optional(),
  type: z19.string(),
  coverageType: z19.string().optional(),
  excerpt: z19.string().optional(),
  content: z19.string().optional(),
  subsections: z19.array(SubsectionSchema).optional(),
  recordId: z19.string().optional(),
  documentNodeId: z19.string().optional(),
  sourceSpanIds: z19.array(z19.string()).optional(),
  sourceTextHash: z19.string().optional()
});
var SubjectivitySchema = z19.object({
  description: z19.string(),
  category: z19.string().optional()
});
var UnderwritingConditionSchema = z19.object({
  description: z19.string()
});
var PremiumLineSchema = z19.object({
  line: z19.string(),
  amount: z19.string(),
  amountValue: z19.number().optional(),
  documentNodeId: z19.string().optional(),
  sourceSpanIds: z19.array(z19.string()).optional(),
  sourceTextHash: z19.string().optional()
});
var AuxiliaryFactSchema = z19.object({
  key: z19.string(),
  value: z19.string(),
...
var DefinitionSchema = z19.object({
  term: z19.string(),
  definition: z19.string(),
  pageNumber: z19.number().optional(),
  formNumber: z19.string().optional(),
  formTitle: z19.string().optional(),
  sectionRef: z19.string().optional(),
  originalContent: z19.string().optional(),
  recordId: z19.string().optional(),
  documentNodeId: z19.string().optional(),
  sourceSpanIds: z19.array(z19.string()).optional(),
  sourceTextHash: z19.string().optional()
});
var CoveredReasonSchema = z19.object({
  coverageName: z19.string(),
  reasonNumber: z19.string().optional(),
  title: z19.string().optional(),
  content: z19.string(),
  conditions: z19.array(z19.string()).optional(),
  exceptions: z19.array(z19.string()).optional(),
  appliesTo: z19.array(z19.string()).optional(),
  pageNumber: z19.number().optional(),
  formNumber: z19.string().optional(),
  formTitle: z19.string().optional(),
  sectionRef: z19.string().optional(),
...
var DocumentTableOfContentsEntrySchema = z19.object({
  title: z19.string(),
  level: z19.number().int().positive().optional(),
  pageStart: z19.number().optional(),
  pageEnd: z19.number().optional(),
  documentNodeId: z19.string().optional(),
  sourceSpanIds: z19.array(z19.string()).optional()
});
var DocumentPageMapEntrySchema = z19.object({
  page: z19.number().int().positive(),
  label: z19.string().optional(),
  formNumber: z19.string().optional(),
  formTitle: z19.string().optional(),
  sectionTitle: z19.string().optional(),
  extractorNames: z19.array(z19.string()).optional(),
  sourceSpanIds: z19.array(z19.string()).optional()
});
var DocumentAgentGuidanceSchema = z19.object({
  kind: z19.string(),
  title: z19.string(),
  detail: z19.string(),
  sourceSpanIds: z19.array(z19.string()).optional()
});
var DocumentMetadataSchema = z19.object({
  sourceTreeVersion: z19.string().optional(),
  sourceTreeCanonical: z19.boolean().optional(),
  formInventory: z19.array(FormReferenceSchema).optional(),
  tableOfContents: z19.array(DocumentTableOfContentsEntrySchema).optional(),
  pageMap: z19.array(DocumentPageMapEntrySchema).optional(),
  agentGuidance: z19.array(DocumentAgentGuidanceSchema).optional()
});
var DocumentNodeSchema = z19.lazy(
  () => z19.object({
    id: z19.string(),
    title: z19.string(),
    originalTitle: z19.string().optional(),
    ...
    excerpt: z19.string().optional(),
    content: z19.string().optional(),
    interpretationLabels: z19.array(z19.string()).optional(),
    sourceSpanIds: z19.array(z19.string()).optional(),
    sourceTextHash: z19.string().optional(),
    children: z19.array(DocumentNodeSchema).optional()
  })
);
var BaseDocumentFields = {
  id: z19.string(),
  carrier: z19.string(),
  security: z19.string().optional(),
  insuredName: z19.string(),
  premium: z19.string().optional(),
  premiumAmount: z19.number().optional(),
  premiumBreakdown: z19.array(PremiumLineSchema).optional(),
  summary: z19.string().optional(),
  linesOfBusiness: z19.array(z19.string()).optional(),
  coverages: z19.array(CoverageSchema),
  documentMetadata: DocumentMetadataSchema,
  documentOutline: z19.array(DocumentNodeSchema),
  sections: z19.array(SectionSchema).optional(),
  definitions: z19.array(DefinitionSchema).optional(),
  coveredReasons: z19.array(CoveredReasonSchema).optional(),
  // Enriched fields (v1.2+)
  carrierLegalName: z19.string().optional(),
  carrierNaicNumber: z19.string().optional(),
  carrierAmBestRating: z19.string().optional(),
  carrierAdmittedStatus: z19.string().optional(),
  mga: z19.string().optional(),
  underwriter: z19.string().optional(),
  brokerAgency: z19.string().optional(),
  brokerContactName: z19.string().optional(),
  brokerLicenseNumber: z19.string().optional(),
  priorPolicyNumber: z19.string().optional(),
  programName: z19.string().optional(),
  isRenewal: z19.boolean().optional(),
  isPackage: z19.boolean().optional(),
  insuredDba: z19.string().optional(),
  insuredAddress: SourceBackedAddressSchema.optional(),
  insuredEntityType: EntityTypeSchema.optional(),
  additionalNamedInsureds: z19.array(NamedInsuredSchema).optional(),
  insuredSicCode: z19.string().optional(),
  insuredNaicsCode: z19.string().optional(),
  insuredFein: z19.string().optional(),
  enrichedCoverages: z19.array(EnrichedCoverageSchema).optional(),
  endorsements: z19.array(EndorsementSchema).optional(),
  exclusions: z19.array(ExclusionSchema).optional(),
  conditions: z19.array(PolicyConditionSchema).optional(),
  limits: LimitScheduleSchema.optional(),
  deductibles: DeductibleScheduleSchema.optional(),
  locations: z19.array(InsuredLocationSchema).optional(),
  vehicles: z19.array(InsuredVehicleSchema).optional(),
  classifications: z19.array(ClassificationCodeSchema).optional(),
  coverageSchedules: z19.array(OperationalCoverageScheduleSchema).optional(),
  formInventory: z19.array(FormReferenceSchema).optional(),
  declarations: DeclarationsSchema.optional(),
  coverageForm: CoverageFormSchema.optional(),
  retroactiveDate: z19.string().optional(),
  extendedReportingPeriod: ExtendedReportingPeriodSchema.optional(),
  insurer: InsurerInfoSchema.optional(),
  producer: ProducerInfoSchema.optional(),
  claimsContacts: z19.array(ContactSchema).optional(),
  regulatoryContacts: z19.array(ContactSchema).optional(),
  thirdPartyAdministrators: z19.array(ContactSchema).optional(),
  additionalInsureds: z19.array(EndorsementPartySchema).optional(),
  lossPayees: z19.array(EndorsementPartySchema).optional(),
  mortgageHolders: z19.array(EndorsementPartySchema).optional(),
  taxesAndFees: z19.array(TaxFeeItemSchema).optional(),
  totalCost: z19.string().optional(),
  totalCostAmount: z19.number().optional(),
  minimumPremium: z19.string().optional(),
  minimumPremiumAmount: z19.number().optional(),
  depositPremium: z19.string().optional(),
  depositPremiumAmount: z19.number().optional(),
  paymentPlan: PaymentPlanSchema.optional(),
  auditType: AuditTypeSchema.optional(),
  ratingBasis: z19.array(RatingBasisSchema).optional(),
  premiumByLocation: z19.array(LocationPremiumSchema).optional(),
  lossSummary: LossSummarySchema.optional(),
  individualClaims: z19.array(ClaimRecordSchema).optional(),
  experienceMod: ExperienceModSchema.optional(),
  cancellationNoticeDays: z19.number().optional(),
  nonrenewalNoticeDays: z19.number().optional(),
  supplementaryFacts: z19.array(AuxiliaryFactSchema).optional()
};
var PolicyDocumentSchema = z19.object({
  ...BaseDocumentFields,
  type: z19.literal("policy"),
  policyNumber: z19.string(),
  effectiveDate: z19.string(),
  expirationDate: z19.string().optional(),
  policyTermType: PolicyTermTypeSchema.optional(),
  nextReviewDate: z19.string().optional(),
  effectiveTime: z19.string().optional()
});
var QuoteDocumentSchema = z19.object({
  ...BaseDocumentFields,
  type: z19.literal("quote"),
  quoteNumber: z19.string(),
  proposedEffectiveDate: z19.string().optional(),
  proposedExpirationDate: z19.string().optional(),
  quoteExpirationDate: z19.string().optional(),
  subjectivities: z19.array(SubjectivitySchema).optional(),
  underwritingConditions: z19.array(UnderwritingConditionSchema).optional(),
  // Enriched quote fields (v1.2+)
  enrichedSubjectivities: z19.array(EnrichedSubjectivitySchema).optional(),
  enrichedUnderwritingConditions: z19.array(EnrichedUnderwritingConditionSchema).optional(),
  warrantyRequirements: z19.array(z19.string()).optional(),
  lossControlRecommendations: z19.array(z19.string()).optional(),
  bindingAuthority: BindingAuthoritySchema.optional()
});
var InsuranceDocumentSchema = z19.discriminatedUnion("type", [
...
var PlatformSchema = z20.enum(["email", "chat", "sms", "slack", "discord"]);
var CommunicationIntentSchema = z20.enum(["direct", "mediated", "observed"]);
...
var CaseEvidenceSourceSchema = z21.object({
  id: z21.string(),
  label: z21.string().optional(),
  documentId: z21.string().optional(),
  page: z21.number().optional(),
  fieldPath: z21.string().optional(),
  text: z21.string().describe("Source text available for span validation and citation"),
  metadata: z21.record(z21.string(), z21.string()).optional()
});
var CaseCitationSchema = z21.object({
  sourceId: z21.string(),
  quote: z21.string(),
  page: z21.number().optional(),
  fieldPath: z21.string().optional()
});
var ValidationIssueSeveritySchema = z21.enum(["info", "warning", "blocking"]);
var CaseValidationIssueSchema = z21.object({
  code: z21.string(),
  severity: ValidationIssueSeveritySchema,
  message: z21.string(),
  itemId: z21.string().optional(),
  fieldPath: z21.string().optional(),
  sourceId: z21.string().optional()
});
var MissingInfoQuestionSchema = z21.object({
  id: z21.string(),
  itemId: z21.string().optional(),
  fieldPath: z21.string().optional(),
  question: z21.string(),
  reason: z21.string(),
  answer: z21.string().optional()
});
var CasePacketArtifactKindSchema = z21.enum([
  "underwriter_summary",
  "carrier_email",
...
var CasePacketArtifactSchema = z21.object({
  id: z21.string(),
...
var CaseSubmissionPacketSchema = z21.object({
  id: z21.string(),
  caseId: z21.string(),
...
var AgenticExecutionModeSchema = z21.enum(["deterministic_tree", "market_eval", "hybrid"]);
var CaseProposalScoreSchema = z21.object({
  grounding: z21.number().min(0).max(1),
  completeness: z21.number().min(0).max(1),
  consistency: z21.number().min(0).max(1),
  determinism: z21.number().min(0).max(1),
  risk: z21.number().min(0).max(1),
  cost: z21.number().min(0).max(1)
});
var CaseProposalSchema = z21.object({
  id: z21.string(),
  sourceSpanIds: z21.array(z21.string()).default([]),
  confidence: z21.number().min(0).max(1),
  missingInfo: z21.array(z21.string()).default([]),
  validationIssues: z21.array(CaseValidationIssueSchema).default([]),
  estimatedRisk: z21.number().min(0).max(1).default(0.5),
  estimatedCost: z21.number().min(0).max(1).default(0.5),
...
function validateQuotedEvidence(params) {
  ...
  const source = params.sources.find((candidate2) => candidate2.id === citation.sourceId);
  if (!source) {
    return [{
      code: "unknown_source",
      severity: params.severity ?? "blocking",
...
function mergeQuestionAnswers(questions, answers) {
  ...
  const merged = questions.map((question) => {
    const answer = answers.find(
      (candidate2) => candidate2.questionId && candidate2.questionId === question.id || candidate2.fieldPath && candidate2.fieldPath === question.fieldPath
    );
...
// src/schemas/pce.ts
var PolicyChangeActionSchema = z22.enum(["add", "remove", "update", "replace", "clarify"]);
var PolicyChangeKindSchema = z22.enum([
...
var PolicyChangeConfidenceSchema = z22.enum(["high", "medium", "low"]);
var PolicyChangeStatusSchema = z22.enum(["draft", "needs_info", "ready", "blocked"]);
var PolicyChangeItemSchema = z22.object({
  id: z22.string(),
  kind: PolicyChangeKindSchema.default("general_endorsement"),
  action: PolicyChangeActionSchema,
  affectedPolicyId: z22.string().default("unknown"),
  fieldPath: z22.string().describe("Stable policy field path or business field name"),
  label: z22.string(),
  beforeValue: z22.string().optional().describe("Existing policy value, when cited from policy evidence"),
  afterValue: z22.string().optional().describe("Requested new value"),
  requestedValue: z22.string().optional().describe("Alias for afterValue used by policy-change workflows"),
  effectiveDate: z22.string().optional(),
  reason: z22.string().optional(),
  sourceIds: z22.array(z22.string()).default([]),
  sourceSpanIds: z22.array(z22.string()).default([]),
  userSourceSpanIds: z22.array(z22.string()).optional(),
  citations: z22.array(CaseCitationSchema).default([]),
  confidence: PolicyChangeConfidenceSchema.default("medium"),
  confidenceScore: z22.number().min(0).max(1).optional(),
  status: PolicyChangeStatusSchema.default("ready")
});
var PceNormalizationResultSchema = z22.object({
  summary: z22.string(),
  items: z22.array(PolicyChangeItemSchema.omit({ id: true, status: true }).extend({
    id: z22.string().optional(),
    status: PolicyChangeStatusSchema.optional()
  })),
  missingInfoQuestions: z22.array(MissingInfoQuestionSchema.omit({ id: true }).extend({
    id: z22.string().optional()
  })).default([])
});
var PolicyChangeImpactSchema = z22.object({
  itemId: z22.string(),
  beforeValue: z22.string().optional(),
  requestedValue: z22.string().optional(),
  likelyEndorsementRequired: z22.boolean().default(true),
  carrierApprovalLikelyRequired: z22.boolean().default(true),
  affectedCoverageForms: z22.array(z22.string()).default([]),
  sourceSpanIds: z22.array(z22.string()).default([])
});
var PceCaseStateSchema = z22.object({
  id: z22.string(),
  requestText: z22.string(),
  summary: z22.string(),
  executionMode: AgenticExecutionModeSchema.default("deterministic_tree"),
  items: z22.array(PolicyChangeItemSchema),
  impacts: z22.array(PolicyChangeImpactSchema),
  evidenceSources: z22.array(CaseEvidenceSourceSchema),
  validationIssues: z22.array(CaseValidationIssueSchema),
  missingInfoQuestions: z22.array(MissingInfoQuestionSchema),
  createdAt: z22.number(),
  updatedAt: z22.number()
});
var PolicyChangeRequestSchema = z22.object({
  id: z22.string(),
  text: z22.string(),
...
var PceSubmissionPacketSchema = CaseSubmissionPacketSchema.extend({
  ...
  artifacts: z22.array(CasePacketArtifactSchema)
});
...
// src/source/evidence-ledger.ts
var CORE_CONTEXT = /\b(?:policy\s*(?:number|no\.?|#)|named\s+insured|insured\s+name|carrier|insurer|insurance\s+company|policy\s+period|effective\s+date|expiration\s+date|expiry\s+date|producer|broker|general\s+agent|declarations?)\b/i;
var COVERAGE_CONTEXT = /\b(?:coverage|coverages|covered|limit(?:s)?\s+of\s+(?:insurance|liability)|deductible|retention|insuring\s+agreement|schedule\s+of|vehicle\s+schedule|auto\s+schedule|property\s+schedule|location\s+schedule|coverage\s+part|premium\s+schedule)\b/i;
var PARTY_CHANGING_ENDORSEMENT = /\b(?:endorsement|additional\s+insured|named\s+insured|loss\s+payee|mortgagee|carrier|insurer|producer|broker|general\s+agent|changes?\s+the\s+policy)\b/i;
var FIELD_PATTERNS = {
  policy_number: [
...
function normalizedCandidate(field, value) {
  const cleaned = cleanText(value).replace(/^[\s:#-]+/, "").replace(/[\s|;,]+$/, "").trim();
...
function plausibleValue(field, value) {
...
function spanOrdinal(span) {
  const match = span.id.match(/:span:[^:]+:(\d+):[^:]+$/);
  if (!match?.[1]) return void 0;
  const value = Number(match[1]);
  return Number.isSafeInteger(value) ? value : void 0;
}
function eligibleSpans(sourceSpans) {
  return sourceSpans.map((span, inputIndex) => ({ span, inputIndex })).filter(({ span }) => cleanText(span.text).length > 0).sort((left, right) => {
    const pageOrder = (spanPageStart(left.span) ?? Number.MAX_SAFE_INTEGER) - (spanPageStart(right.span) ?? Number.MAX_SAFE_INTEGER);
    ...
    if (leftTableId && leftTableId === rightTableId) {
      const rowOrder = (left.span.table?.rowIndex ?? 0) - (right.span.table?.rowIndex ?? 0);
      if (rowOrder !== 0) return rowOrder;
      const columnOrder = (left.span.table?.columnIndex ?? 0) - (right.span.table?.columnIndex ?? 0);
      if (columnOrder !== 0) return columnOrder;
    }
    const lineOrder = (left.span.location?.lineStart ?? Number.MAX_SAFE_INTEGER) - (right.span.location?.lineStart ?? Number.MAX_SAFE_INTEGER);
    if (lineOrder !== 0) return lineOrder;
    const characterOrder = (left.span.location?.charStart ?? Number.MAX_SAFE_INTEGER) - (right.span.location?.charStart ?? Number.MAX_SAFE_INTEGER);
    if (characterOrder !== 0) return characterOrder;
    const ordinalOrder = (spanOrdinal(left.span) ?? Number.MAX_SAFE_INTEGER) - (spanOrdinal(right.span) ?? Number.MAX_SAFE_INTEGER);
    return ordinalOrder || left.inputIndex - right.inputIndex;
  }).map(({ span }) => span);
}
function fingerprint(spans) {
  return stableHash2(spans.map((span) => ({
    id: span.id,
...
function projections(sourceSpans, sourceTree) {
  const nodesBySpan = /* @__PURE__ */ new Map();
  ...
  return eligibleSpans(sourceSpans).map((span) => {
    const nodes = (nodesBySpan.get(span.id) ?? []).slice().sort((left, right) => left.order - right.order || compareText(left.id, right.id));
    return {
      span,
      nodes,
      context: cleanText([
...
function dedupeCandidates(candidates) {
  ...
  for (const item of candidates) {
    const key = `${item.normalizedValue}\0${item.sourceSpanIds.join(",")}`;
    if (!byKey.has(key)) byKey.set(key, item);
  }
  return [...byKey.values()].sort((left, right) => (left.pageStart ?? Number.MAX_SAFE_INTEGER) - (right.pageStart ?? Number.MAX_SAFE_INTEGER) || compareText(left.normalizedValue, right.normalizedValue) || compareText(left.sourceSpanIds.join(","), right.sourceSpanIds.join(",")));
}
function inlineCandidates(field, projected) {
  const values = [];
...
function adjacentCandidates(field, projected) {
  const values = [];
  for (let index = 0; index < projected.length - 1; index += 1) {
    ...
    if (labelPage !== void 0 && valuePage !== void 0 && labelPage !== valuePage) continue;
    const sameRow = Boolean(
      label.span.table?.rowSpanId && label.span.table.rowSpanId === value.span.table?.rowSpanId
    );
...
function fieldResult(field, projected) {
...
function coverageRegions(projected) {
  const regions = [];
  for (const projection of projected) {
  ...
  return regions.filter((region) => {
    ...
    seen.add(key);
    return true;
  }).sort((left, right) => (left.pageStart ?? Number.MAX_SAFE_INTEGER) - (right.pageStart ?? Number.MAX_SAFE_INTEGER) || compareText(left.sourceSpanIds[0] ?? "", right.sourceSpanIds[0] ?? ""));
}
function buildExtractionSourceCoverageMap(sourceSpans, sourceTree) {
  const projected = projections(sourceSpans, sourceTree);
  const entries = projected.map(({ span, nodes }) => {
    ...
    const inEndorsement = nodes.some((node) => node.kind === "endorsement" || /\bendorsement\b/i.test(`${node.title} ${node.path}`));
    const core = CORE_CONTEXT.test(context);
    const coverage = COVERAGE_CONTEXT.test(context);
    const partyChangingEndorsement = inEndorsement && PARTY_CHANGING_ENDORSEMENT.test(context);
    const assignment = partyChangingEndorsement || core && coverage ? "both" : core ? "core" : coverage ? "coverage" : "catch_all";
...
function buildExtractionEvidenceLedger(sourceSpans, sourceTree, options = {}) {
  const projected = projections(sourceSpans, sourceTree);
  const eligibleSourceSpanIds = projected.map(({ span }) => span.id);
  const projectedSourceSpanIds = projected.filter(({ nodes }) => nodes.length > 0).map(({ span }) => span.id);
  const processed = options.processedSourceSpanIds ? new Set(options.processedSourceSpanIds) : void 0;
  const completeSourceCoverage = eligibleSourceSpanIds.length > 0 && projectedSourceSpanIds.length === eligibleSourceSpanIds.length && (!processed || eligibleSourceSpanIds.every((sourceSpanId) => processed.has(sourceSpanId)));
  const fields = {
  ...
  const ledgerWithoutHash = {
    ...
    fields,
    coverageRegions: {
      status: regions.length ? "observed" : "not_observed",
      candidates: regions
    },
  ...
  return {
    ...ledgerWithoutHash,
    ledgerHash: stableHash2(ledgerWithoutHash)
  };
}
...
function mergeOperationalProfile(base, candidate2, validNodeIds, validSpanIds) {
  ...
  const policyNumber = mergeValue(base.policyNumber, candidate2.policyNumber);
  const namedInsured = mergeValue(base.namedInsured, candidate2.namedInsured);
  const insurer = mergeValue(base.insurer, candidate2.insurer);
  const broker = mergeValue(base.broker, candidate2.broker);
  const effectiveDate = mergeValue(base.effectiveDate, candidate2.effectiveDate);
  const expirationDate = mergeValue(base.expirationDate, candidate2.expirationDate);
  const retroactiveDate = mergeValue(base.retroactiveDate, candidate2.retroactiveDate);
  const premium = mergeValue(base.premium, candidate2.premium);
  const operationsDescription = mergeValue(base.operationsDescription, candidate2.operationsDescription);
  const productName = mergeValue(base.productIdentity?.name, candidate2.productIdentity?.name);
  const companyProductCode = mergeValue(
    base.productIdentity?.companyProductCode,
    candidate2.productIdentity?.companyProductCode
  );
  const companyProductSubCode = mergeValue(
    base.productIdentity?.companyProductSubCode,
    candidate2.productIdentity?.companyProductSubCode
  );
  ...
  const candidateDeclarationFacts = Array.isArray(candidate2.declarationFacts) ? candidate2.declarationFacts.map(mergeDeclarationFact).filter((fact) => Boolean(fact)) : [];
  ...
  const coverages = base.coverages.length > 0 ? base.coverages : Array.isArray(candidate2.coverages) ? candidate2.coverages.map((coverage) => {
  ...
  const candidateParties = Array.isArray(candidate2.parties) ? candidate2.parties.flatMap((party) => {
  ...
  const endorsementSupport = [
    ...
    ...Array.isArray(candidate2.endorsementSupport) ? candidate2.endorsementSupport.flatMap((row) => {
  ...
  const sourceNodeIds = [.../* @__PURE__ */ new Set([
    ...base.sourceNodeIds,
    ...keepIds(candidate2.sourceNodeIds, validNodeIds),
  ...
  const sourceSpanIds = [.../* @__PURE__ */ new Set([
    ...base.sourceSpanIds,
    ...keepIds(candidate2.sourceSpanIds, validSpanIds),
  ...
  const warnings = [
    ...base.warnings,
    ...Array.isArray(candidate2.warnings) ? candidate2.warnings.filter((warning) => typeof warning === "string" && warning.trim().length > 0) : []
  ];
  const candidateLinesOfBusiness = candidate2.linesOfBusiness ?? candidate2.policyTypes;
  ...
  return PolicyOperationalProfileSchema.parse({
    ...base,
    documentType: candidate2.documentType === "policy" ? "policy" : base.documentType,
...
var OperationalCoverageTermKindSchema = z23.enum(OPERATIONAL_COVERAGE_TERM_KINDS2);
var OperationalProfileCleanupSchema = z23.object({
  coverageDecisions: z23.array(z23.object({
    coverageIndex: z23.number().int().nonnegative(),
    action: z23.enum(["keep", "drop", "update"]),
    reason: z23.string().optional(),
    name: z23.string().optional(),
    lineOfBusiness: z23.string().nullable().optional(),
    limit: z23.string().nullable().optional(),
    deductible: z23.string().nullable().optional(),
    premium: z23.string().nullable().optional(),
    retroactiveDate: z23.string().nullable().optional(),
    sourceNodeIds: z23.array(z23.string()).optional(),
    sourceSpanIds: z23.array(z23.string()).optional(),
    termDecisions: z23.array(z23.object({
      termIndex: z23.number().int().nonnegative(),
      action: z23.enum(["keep", "drop", "update"]),
      reason: z23.string().optional(),
      kind: OperationalCoverageTermKindSchema.optional(),
      label: z23.string().optional(),
      value: z23.string().optional(),
      amount: z23.number().nullable().optional(),
      appliesTo: z23.string().nullable().optional(),
      sourceNodeIds: z23.array(z23.string()).optional(),
      sourceSpanIds: z23.array(z23.string()).optional()
    })).optional()
  })).default([]),
...
var RecoveryRegionSchema = z24.object({
  pageStart: z24.number().int().positive(),
  pageEnd: z24.number().int().positive(),
  reason: z24.string(),
  priorContext: z24.string().optional(),
  sourceNodeIds: z24.array(z24.string()).default([]),
  sourceSpanIds: z24.array(z24.string()).default([])
});
var RecoveryRegionDiscoverySchema = z24.object({
  regions: z24.array(RecoveryRegionSchema).default([]),
  warnings: z24.array(z24.string()).default([])
});
var CoverageRecoveryCandidateSchema = z24.object({
  coverages: z24.array(OperationalCoverageLineSchema).default([]),
  coverageSchedules: z24.array(OperationalCoverageScheduleSchema).default([]),
  premiumBreakdown: z24.array(OperationalPremiumLineSchema).default([]),
  taxesAndFees: z24.array(OperationalTaxFeeItemSchema).default([]),
  totalCost: SourceBackedValueSchema.optional(),
  warnings: z24.array(z24.string()).default([])
});
...
function spanPageEnd(span) {
  return span.pageEnd ?? span.location?.endPage ?? span.location?.page ?? spanPageStart2(span);
}
...
function documentPages(sourceTree, sourceSpans) {
  ...
  for (const span of sourceSpans) {
    const start = spanPageStart2(span);
...
function pageSketch(page, sourceTree, sourceSpans) {
  ...
  const pageSpans = sourceSpans.filter((span) => overlapsPageRange({ pageStart: spanPageStart2(span), pageEnd: spanPageEnd(span) }, page, page)).sort(
  ...
  const textSamples = evenlySample(
    pageSpans.filter((span) => cleanText2(span.text)).map((span) => ({
      ...
      bbox: span.bbox?.[0],
      text: cleanText2(span.text)?.slice(0, 280)
    })),
    18
  );
  return {
...
function priorStructuralContext(region, sourceTree, sourceSpans) {
  ...
  const headerSpans = sourceSpans.filter((span) => {
    ...
    table: span.table,
    text: cleanText2(span.text)?.slice(0, 600)
  }));
...
function regionEvidence(region, sourceTree, sourceSpans) {
  ...
  const spans = sourceSpans.filter((span) => overlapsPageRange(
    { pageStart: spanPageStart2(span), pageEnd: spanPageEnd(span) },
    region.pageStart,
    region.pageEnd
  )).sort(
...
function validateCandidate(candidate2, sourceTree, sourceSpans) {
  ...
  const coverages = candidate2.coverages.flatMap((coverage) => {
  ...
  const coverageSchedules = candidate2.coverageSchedules.flatMap((schedule) => {
  ...
  const premiumBreakdown = candidate2.premiumBreakdown.flatMap((row) => {
    const citation = validateCitation(row.sourceNodeIds, row.sourceSpanIds, [row.line, row.amount]);
    return citation ? [{ ...row, ...citation }] : [];
  });
  const taxesAndFees = candidate2.taxesAndFees.flatMap((row) => {
    const citation = validateCitation(row.sourceNodeIds, row.sourceSpanIds, [row.name, row.amount]);
    return citation ? [{ ...row, ...citation }] : [];
  });
  const totalCostCitation = candidate2.totalCost ? validateCitation(candidate2.totalCost.sourceNodeIds, candidate2.totalCost.sourceSpanIds, [candidate2.totalCost.value]) : void 0;
  return {
    candidate: {
      ...candidate2,
      coverages,
      ...
      premiumBreakdown,
      taxesAndFees,
      totalCost: candidate2.totalCost && totalCostCitation ? { ...candidate2.totalCost, ...totalCostCitation } : void 0
    },
...
function sourcePages(ids, spansById) {
  const pages = ids.flatMap((id) => {
    ...
    const start = span ? spanPageStart2(span) : void 0;
...
function sameCoverageScope(primary, recovery, spansById) {
  ...
  for (const key of ["formNumber", "sectionRef", "endorsementNumber"]) {
    const left = cleanText2(primary[key]);
    const right = cleanText2(recovery[key]);
...
function mergeRecoveryCandidate(profile, candidate2, sourceSpans) {
  const warnings = [...candidate2.warnings];
  const spansById = new Map(sourceSpans.map((span) => [span.id, span]));
  const coverages = [...profile.coverages];
  let recoveredCoverageCount = 0;
  let recoveredTermCount = 0;
  for (const recovery of candidate2.coverages) {
...
// src/extraction/source-tree-extractor.ts
var SourceBackedValueForPromptSchema = z25.object({
  value: z25.string(),
  normalizedValue: z25.string().optional(),
  confidence: z25.enum(["low", "medium", "high"]).optional(),
  sourceNodeIds: z25.array(z25.string()),
  sourceSpanIds: z25.array(z25.string())
});
var OperationalAddressForPromptSchema = z25.object({
  street1: z25.string().optional(),
  street2: z25.string().optional(),
  city: z25.string().optional(),
  state: z25.string().optional(),
  zip: z25.string().optional(),
  country: z25.string().optional(),
  formatted: z25.string().optional()
});
var OperationalDeclarationFactForPromptSchema = z25.object({
  field: z25.enum([
    "namedInsured",
    "mailingAddress",
    "dba",
  ...
  value: z25.string(),
  normalizedValue: z25.string().optional(),
  valueKind: z25.enum(["string", "number", "date", "money", "address", "list", "unknown"]).optional(),
  address: OperationalAddressForPromptSchema.optional(),
  confidence: z25.enum(["low", "medium", "high"]).optional(),
  sourceNodeIds: z25.array(z25.string()),
  sourceSpanIds: z25.array(z25.string())
});
var OperationalPartyForPromptSchema = z25.object({
  role: z25.enum([
    "named_insured",
    "producer",
    "broker",
...
var OperationalProfilePromptSchema = z25.object({
  documentType: z25.enum(["policy", "quote"]).optional(),
  linesOfBusiness: z25.array(z25.string()).optional(),
  ...
  declarationFacts: z25.array(OperationalDeclarationFactForPromptSchema).optional(),
  parties: z25.array(OperationalPartyForPromptSchema).optional(),
  coverages: z25.array(z25.object({
    name: z25.string(),
    lineOfBusiness: z25.string().optional(),
    coverageCode: z25.string().optional(),
    limit: z25.string().optional(),
    deductible: z25.string().optional(),
    premium: z25.string().optional(),
    retroactiveDate: z25.string().optional(),
    formNumber: z25.string().optional(),
    sectionRef: z25.string().optional(),
    endorsementNumber: z25.string().optional(),
    limits: z25.array(z25.object({
      kind: OperationalCoverageTermKindSchema.optional(),
      label: z25.string(),
      value: z25.string(),
      amount: z25.number().optional(),
      appliesTo: z25.string().optional(),
      sourceNodeIds: z25.array(z25.string()),
      sourceSpanIds: z25.array(z25.string())
    })).optional(),
    sourceNodeIds: z25.array(z25.string()),
  ...
  sourceNodeIds: z25.array(z25.string()).optional(),
  sourceSpanIds: z25.array(z25.string()).optional()
});
function cleanText3(value, fallback) {
  const text = value?.replace(/\s+/g, " ").trim();
  return text || fallback;
}
function simplifyOrganizerTitle(value, fallback, kind) {
  const title = cleanText3(value, fallback);
...
function sourceNodeText(node) {
  return cleanText3([node.title, node.description, node.textExcerpt].filter(Boolean).join(" "), "");
}
function looksLikeEndorsementStart(node) {
  const title = cleanText3(node.title, "");
  const body = cleanText3([node.textExcerpt, node.description].filter(Boolean).join(" "), "");
...
function endorsementTitleKey(node) {
  ...
  const fallback = cleanText3(node.title, "");
...
function spanPageStart3(span) {
  return span.pageStart ?? span.location?.page ?? span.location?.startPage;
}
function spanPageEnd2(span) {
  return span.pageEnd ?? span.location?.endPage ?? spanPageStart3(span);
}
function spanSourceUnit2(span) {
  return span.sourceUnit ?? span.metadata?.sourceUnit ?? span.metadata?.elementType;
}
function pageHeadingTitleFromText(text, fallback) {
  const normalized = cleanText3(text, "");
  ...
  for (const pattern of patterns) {
    ...
    if (match) return cleanText3(match, fallback);
  }
  return fallback;
}
...
function looksLikeDeclarationsStart(node) {
  ...
  return /^declarations?$/i.test(title) || /\bdeclarations?\s+(page|schedule|section)\b/i.test(text) || /^declarations?\b/i.test(cleanText3(node.textExcerpt, ""));
}
...
function looksLikePolicyFormStart(node) {
  const text = sourceNodeText(node);
  const excerpt = cleanText3(node.textExcerpt, "");
  ...
  const relabeled = sourceTree.map((node) => {
    ...
    if (nextNode.kind === "page" && looksLikeDeclarationsStart(nextNode)) {
      return {
        ...nextNode,
        title: "Declarations",
        description: cleanText3([nextNode.description, "Declarations"].join(" "), "Declarations"),
        metadata: { ...nextNode.metadata, organizerRepair: "semantic_page_grouping" }
      };
    }
    if (nextNode.kind === "page" && looksLikePolicyFormStart(nextNode)) {
      return {
        ...nextNode,
        title: "Policy Form",
        description: cleanText3([nextNode.description, "Policy Form"].join(" "), "Policy Form"),
        metadata: { ...nextNode.metadata, organizerRepair: "semantic_page_grouping" }
      };
    }
    return nextNode;
...
function sectionHeadingTitle(node, container) {
  ...
  const text = cleanText3(node.title || node.textExcerpt, "");
    ...
    for (let index = 0; index < directPageChildren.length; index += 1) {
      ...
      if (heading) {
        ...
        updates.set(child.id, {
          ...
          title: heading,
          description: descriptionWithPages(cleanText3([heading, "section"].join(" "), heading), [current, ...descendants]),
          metadata: {
            ...current.metadata,
...
function applyEndorsementGrouping(sourceTree) {
  ...
  const relabeledTree = sourceTree.map((node) => {
    ...
    if (!title && node.kind === "endorsement") {
      return {
        ...node,
        kind: "page",
        title: node.pageStart ? `Page ${node.pageStart}` : cleanText3(node.title, "Page"),
        metadata: {
          ...node.metadata,
  ...
  let nextTree = relabeledTree.map((node) => {
    ...
    const normalized = {
      ...
      kind: "page_group",
      title: "Endorsements",
      description: descriptionWithPages(cleanText3(node.description, "Endorsement forms grouped by source order"), byParent.get(node.id) ?? [node]),
      metadata: {
        ...node.metadata,
        sourceTreeVersion: "v3",
...
function isOperationalEvidenceAnchor(span) {
  ...
  const text = cleanText3([span.text, span.formNumber].filter(Boolean).join(" "), "");
...
function operationalEvidencePages(sourceSpans) {
  ...
  for (const span of sourceSpans) {
    const page = spanPageStart3(span);
    if (typeof page !== "number") continue;
    const text = cleanText3([span.text, span.formNumber, spanSourceUnit2(span)].filter(Boolean).join(" "), "");
    if (!text) continue;
...
function operationalProfileEvidence(sourceTree, sourceSpans) {
  const sorted = [...sourceSpans].sort(
    (left, right) => (spanPageStart3(left) ?? Number.MAX_SAFE_INTEGER) - (spanPageStart3(right) ?? Number.MAX_SAFE_INTEGER) || (left.location?.charStart ?? Number.MAX_SAFE_INTEGER) - (right.location?.charStart ?? Number.MAX_SAFE_INTEGER) || left.id.localeCompare(right.id)
  );
  ...
  if (selectedTableIds.size > 0) {
    sorted.forEach((span, index) => {
      ...
      const text = cleanText3(span.text, "");
  ...
  const entries = [...selected].sort((left, right) => left - right).map((index) => sorted[index]).filter((span) => !isTableCellSpan(span)).flatMap((span) => {
    const text = cleanText3(span.text, "");
    if (!text) return [];
    const key = `${spanPageStart3(span) ?? "na"}:${text.toLowerCase().slice(0, 240)}`;
    if (seenText.has(key)) return [];
    seenText.add(key);
    return [{
      sourceSpanId: span.id,
      sourceNodeIds: [...new Set(nodeIdsBySpanId.get(span.id) ?? [])].slice(0, 4),
      pageStart: spanPageStart3(span),
...
function sectionSourceTree(sourceTree, sourceSpanIds) {
  ...
  return normalizeDocumentSourceTreePaths(sourceTree.filter((node) => included.has(node.id)).map((node) => ({
    ...node,
    sourceSpanIds: node.sourceSpanIds.filter((id) => sourceSpanIds.has(id))
  })));
}
function sectionPrompt(sectionId, sourceTree, sourceSpans) {
  const scope = sectionId === "extraction_policy_core" ? `SECTION CONTRACT \u2014 extraction_policy_core:
...
- Party-changing endorsements are deliberately present here; extract their party effects when cited.` : `SECTION CONTRACT \u2014 extraction_policy_coverage:
...
function sectionProfile(sectionId, output) {
...
function makeSectionResult(value) {
  const withoutHash = {
    version: "extraction-section-result-v1",
    ...value
  };
  return { ...withoutHash, resultHash: stableHash2(withoutHash) };
}
function validStoredSection(result, expected) {
  if (!result || result.version !== "extraction-section-result-v1" || result.sectionId !== expected.sectionId || result.sourceFingerprint !== expected.sourceFingerprint || result.extractorVersion !== expected.extractorVersion || stableHash2(result.sourceSpanIds) !== stableHash2(expected.sourceSpanIds) || result.status === "degraded") {
    return false;
  }
  const { resultHash, ...withoutHash } = result;
  return resultHash === stableHash2(withoutHash);
}
async function saveSectionResult(store, result, log) {
  if (!store) return;
  try {
...
function completionManifest(args) {
  ...
  const processedSourceSpanIds = [...new Set(sectionEntries.filter((section) => section.status !== "degraded").flatMap((section) => section.sourceSpanIds))].sort();
  const withoutHash = {
    ...
    completeSourceCoverage: !sectionEntries.some((section) => section.status === "degraded") && args.eligibleSourceSpanIds.every((id) => processedSourceSpanIds.includes(id)),
    evidenceLedgerHash: args.evidenceLedgerHash
  };
  return { ...withoutHash, manifestHash: stableHash2(withoutHash) };
}
...
function firstOperationalParty(profile, roles) {
  const candidates = roles.flatMap(
    (role) => profile.parties.filter((candidate2) => candidate2.role === role && candidate2.name.trim())
  );
  return candidates.find((candidate2) => candidate2.address) ?? candidates[0];
}
...
function normalizedEntityType(value) {
  const normalized = cleanText3(value, "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
...
function materializeDocument(params) {
  ...
  const base = {
    ...
    linesOfBusiness: profile.linesOfBusiness,
    ...profile.productIdentity?.name ? { programName: profile.productIdentity.name.value } : {},
...
async function runSourceTreeExtraction(params) {
  ...
  if (protocolVersion === "source-tree-v2") {
    ...
    const coreSpanIds = coverageMap.entries.filter((entry) => entry.assignment !== "coverage").map((entry) => entry.sourceSpanId);
    const coverageSpanIds = coverageMap.entries.filter((entry) => entry.assignment === "coverage" || entry.assignment === "both").map((entry) => entry.sourceSpanId);
    const runSection = async (sectionId, sourceSpanIds) => {
      const expected = {
        sectionId,
      ...
      if (sectionId === "extraction_policy_coverage" && evidenceLedger.coverageRegions.status === "not_observed" && evidenceLedger.completeSourceCoverage) {
    ...
    if (params.coverageRecovery?.enabled && !coverage.reused) {
      ...
      const recoveryError = coverageRecovery.status === "failed" ? coverageRecovery.warnings[coverageRecovery.warnings.length - 1] ?? "Coverage recovery failed" : void 0;
      coverage.result = makeSectionResult({
        ...
        ...recoveryError || coverage.result.error ? { error: recoveryError ?? coverage.result.error } : {}
      });
...
// src/extraction/protocol-comparison.ts
var CRITICAL_FACTS = [
  "policyNumber",
  "namedInsured",
  "insurer",
...
function sourceBackedValue(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return void 0;
  const record = value;
  if (typeof record.value !== "string" || !record.value.trim()) return void 0;
  return {
    value: record.value,
    cited: [record.sourceSpanIds, record.sourceNodeIds].some((ids) => Array.isArray(ids) && ids.some((id) => typeof id === "string" && id.length > 0))
  };
}
function coverageMatches(baseline, candidate2) {
  if (candidate2.sourceSpanIds.length === 0 && candidate2.sourceNodeIds.length === 0) {
    return false;
  }
  const baselineSpans = new Set(baseline.sourceSpanIds);
  if (candidate2.sourceSpanIds.some((id) => baselineSpans.has(id))) return true;
  const baselineNodes = new Set(baseline.sourceNodeIds);
  if (candidate2.sourceNodeIds.some((id) => baselineNodes.has(id))) return true;
  const baselineCode = normalize(baseline.coverageCode);
  const candidateCode = normalize(candidate2.coverageCode);
  if (baselineCode && baselineCode === candidateCode) return true;
  return Boolean(normalize(baseline.name) && normalize(baseline.name) === normalize(candidate2.name));
}
function compareExtractionProtocolFixture(fixture) {
  const criticalFactRegressions = [];
  for (const field of CRITICAL_FACTS) {
    const baseline = sourceBackedValue(fixture.v1[field]);
    if (!baseline?.cited) continue;
    const candidate2 = sourceBackedValue(fixture.v2[field]);
    if (!candidate2?.cited) {
      criticalFactRegressions.push(`${field}: missing cited value`);
    } else if (normalize(candidate2.value) !== normalize(baseline.value)) {
      criticalFactRegressions.push(`${field}: value changed`);
    }
  }
  const candidateCoverages = fixture.v2.coverages ?? [];
  const coverageRecallRegressions = (fixture.v1.coverages ?? []).filter((coverage) => (coverage.sourceSpanIds.length > 0 || coverage.sourceNodeIds.length > 0) && !candidateCoverages.some((candidate2) => coverageMatches(coverage, candidate2))).map((coverage) => `${coverage.coverageCode ?? coverage.name}: cited coverage missing`);
  return {
    id: fixture.id,
...
function compareExtractionProtocolCorpus(fixtures) {
...
var ApplicationFieldSchema = z26.object({
  id: z26.string(),
  label: z26.string(),
  section: z26.string(),
  fieldType: FieldTypeSchema,
  required: z26.boolean(),
  options: z26.array(z26.string()).optional(),
  columns: z26.array(z26.string()).optional(),
  requiresExplanationIfYes: z26.boolean().optional(),
  condition: z26.object({
    dependsOn: z26.string(),
    whenValue: z26.string()
  }).optional(),
  value: z26.string().optional(),
  source: z26.string().optional().describe("Where the value came from: auto-fill, user, lookup"),
  confidence: z26.enum(["confirmed", "high", "medium", "low"]).optional(),
  sourceSpanIds: z26.array(z26.string()).optional().describe("Stable source spans that support the field value or field anchor"),
  userSourceSpanIds: z26.array(z26.string()).optional().describe("Message or attachment spans that support user-provided values"),
  pageNumber: z26.number().int().positive().optional().describe("Application page where the field label or anchor appears"),
  fieldAnchorId: z26.string().optional().describe("Stable field anchor ID derived from page, section, label, and form metadata"),
  acroFormName: z26.string().optional().describe("Native PDF AcroForm field name when available"),
  validationStatus: z26.enum(["valid", "needs_review", "unsupported", "missing"]).optional()
});
var ApplicationQuestionConditionSchema = z26.object({
  dependsOn: z26.string(),
  operator: z26.enum(["equals", "not_equals", "in", "not_in", "exists"]).optional(),
  value: z26.string().optional(),
  whenValue: z26.string().optional(),
  values: z26.array(z26.string()).optional()
});
var ApplicationRepeatSchema = z26.object({
  min: z26.number().int().nonnegative().optional(),
  max: z26.number().int().positive().optional(),
  label: z26.string().optional()
});
var ApplicationQuestionNodeSchema = z26.lazy(
  () => z26.object({
    id: z26.string(),
    nodeType: z26.enum(["group", "question", "repeat_group", "table"]),
    fieldId: z26.string().optional(),
    fieldPath: z26.string().optional(),
    parentId: z26.string().optional(),
    order: z26.number().int().nonnegative().optional(),
    label: z26.string(),
    section: z26.string().optional(),
    fieldType: FieldTypeSchema.optional(),
    required: z26.boolean().optional(),
    prompt: z26.string().optional(),
    options: z26.array(z26.string()).optional(),
    columns: z26.array(z26.string()).optional(),
    condition: ApplicationQuestionConditionSchema.optional(),
    repeat: ApplicationRepeatSchema.optional(),
    children: z26.array(ApplicationQuestionNodeSchema).optional()
  })
);
var ApplicationQuestionGraphSchema = z26.object({
  id: z26.string(),
  version: z26.string(),
  title: z26.string().optional(),
  applicationType: z26.string().nullable().optional(),
  source: z26.enum(["pdf", "manual", "imported", "generated"]).default("generated"),
  rootNodeIds: z26.array(z26.string()).optional(),
  nodes: z26.array(ApplicationQuestionNodeSchema)
});
var ApplicationTemplateSchema = z26.object({
  id: z26.string(),
  version: z26.string(),
  title: z26.string(),
  applicationType: z26.string().nullable().optional(),
  questionGraph: ApplicationQuestionGraphSchema,
  fields: z26.array(ApplicationFieldSchema).optional()
});
var ApplicationClassifyResultSchema = z26.object({
  isApplication: z26.boolean(),
  confidence: z26.number().min(0).max(1),
  applicationType: z26.string().nullable()
});
var FieldExtractionResultSchema = z26.object({
  fields: z26.array(ApplicationFieldSchema)
});
var AutoFillMatchSchema = z26.object({
  fieldId: z26.string(),
  value: z26.string(),
  confidence: z26.enum(["confirmed"]),
  contextKey: z26.string()
});
var AutoFillResultSchema = z26.object({
  matches: z26.array(AutoFillMatchSchema)
});
var QuestionBatchResultSchema = z26.object({
  batches: z26.array(z26.array(z26.string()).describe("Array of field IDs in this batch"))
});
var LookupRequestSchema = z26.object({
  type: z26.string().describe("Type of lookup: 'records', 'website', 'policy'"),
  description: z26.string(),
  url: z26.string().optional(),
  targetFieldIds: z26.array(z26.string())
});
var ReplyIntentSchema = z26.object({
  primaryIntent: z26.enum(["answers_only", "question", "lookup_request", "mixed"]),
  hasAnswers: z26.boolean(),
  questionText: z26.string().optional(),
  questionFieldIds: z26.array(z26.string()).optional(),
  lookupRequests: z26.array(LookupRequestSchema).optional()
});
var ParsedAnswerSchema = z26.object({
  fieldId: z26.string(),
  value: z26.string(),
  explanation: z26.string().optional()
});
var AnswerParsingResultSchema = z26.object({
  answers: z26.array(ParsedAnswerSchema),
  unanswered: z26.array(z26.string()).describe("Field IDs that were not answered")
});
var LookupFillSchema = z26.object({
  fieldId: z26.string(),
  value: z26.string(),
  source: z26.string().describe("Specific citable reference, e.g. 'GL Policy #POL-12345 (Hartford)'"),
  sourceSpanIds: z26.array(z26.string()).optional()
});
var LookupFillResultSchema = z26.object({
  fills: z26.array(LookupFillSchema),
  unfillable: z26.array(z26.string()),
  explanation: z26.string().optional()
});
var FlatPdfPlacementSchema = z26.object({
  fieldId: z26.string(),
  page: z26.number(),
  x: z26.number().describe("Percentage from left edge (0-100)"),
  y: z26.number().describe("Percentage from top edge (0-100)"),
  text: z26.string(),
  fontSize: z26.number().optional(),
  isCheckmark: z26.boolean().optional()
});
var AcroFormMappingSchema = z26.object({
  fieldId: z26.string(),
  acroFormName: z26.string(),
  value: z26.string()
});
var QualityGateStatusSchema = z26.enum(["passed", "warning", "failed"]);
var QualitySeveritySchema = z26.enum(["info", "warning", "blocking"]);
var ApplicationQualityIssueSchema = z26.object({
  code: z26.string(),
  severity: QualitySeveritySchema,
  message: z26.string(),
  fieldId: z26.string().optional()
});
var ApplicationQualityRoundSchema = z26.object({
  round: z26.number(),
  kind: z26.string(),
  status: QualityGateStatusSchema,
  summary: z26.string().optional()
});
var ApplicationQualityArtifactSchema = z26.object({
  kind: z26.string(),
  label: z26.string().optional(),
  itemCount: z26.number().optional()
});
var ApplicationEmailReviewSchema = z26.object({
  issues: z26.array(ApplicationQualityIssueSchema),
  qualityGateStatus: QualityGateStatusSchema
});
var ApplicationQualityReportSchema = z26.object({
  issues: z26.array(ApplicationQualityIssueSchema),
  rounds: z26.array(ApplicationQualityRoundSchema).optional(),
  artifacts: z26.array(ApplicationQualityArtifactSchema).optional(),
  emailReview: ApplicationEmailReviewSchema.optional(),
  qualityGateStatus: QualityGateStatusSchema
});
var ApplicationContextProposalSchema = z26.object({
  id: z26.string(),
  fieldId: z26.string().optional(),
  key: z26.string(),
  value: z26.string(),
  category: z26.string(),
  source: z26.enum(["application", "user", "lookup", "policy", "email", "chat", "imessage", "mcp"]),
  confidence: z26.enum(["confirmed", "high", "medium", "low"]),
  sourceSpanIds: z26.array(z26.string()).optional(),
  userSourceSpanIds: z26.array(z26.string()).optional()
});
var ApplicationPacketAnswerSchema = z26.object({
  fieldId: z26.string(),
  label: z26.string(),
  section: z26.string(),
  value: z26.string(),
...
var ApplicationPacketSchema = z26.object({
  id: z26.string(),
  applicationId: z26.string(),
  title: z26.string(),
  status: z26.enum(["draft", "broker_ready", "submitted"]),
  answers: z26.array(ApplicationPacketAnswerSchema),
  missingFieldIds: z26.array(z26.string()),
  qualityReport: ApplicationQualityReportSchema,
  submissionNotes: z26.string().optional(),
  createdAt: z26.number()
});
var ApplicationStateSchema = z26.object({
  id: z26.string(),
  pdfBase64: z26.string().optional().describe("Original PDF, omitted after extraction"),
  templateId: z26.string().optional(),
  templateVersion: z26.string().optional(),
  templateSnapshot: ApplicationTemplateSchema.optional(),
  title: z26.string().optional(),
  applicationType: z26.string().nullable().optional(),
  questionGraph: ApplicationQuestionGraphSchema.optional(),
  fields: z26.array(ApplicationFieldSchema),
  batches: z26.array(z26.array(z26.string())).optional(),
  currentBatchIndex: z26.number().default(0),
  contextProposals: z26.array(ApplicationContextProposalSchema).optional(),
          ...
          try {
            const searchPromises = workflowPlan.documentSearchFields.map(
              (f) => limit(async () => {
                ...
                if (match) {
                  const field = state.fields.find((candidate2) => candidate2.id === f.id);
                  if (field && !field.value) {
                    field.value = match.value;
...
var QueryIntentSchema = z27.enum([
  "policy_question",
  "coverage_comparison",
  "document_search",
  "claims_inquiry",
...
var QueryAttachmentKindSchema = z27.enum(["image", "pdf", "text"]);
var QueryAttachmentSchema = z27.object({
  id: z27.string().optional().describe("Optional stable attachment ID from the caller"),
  kind: QueryAttachmentKindSchema,
  name: z27.string().optional().describe("Original filename or user-facing label"),
  mimeType: z27.string().optional().describe("MIME type such as image/jpeg or application/pdf"),
  base64: z27.string().optional().describe("Base64-encoded file content for image/pdf attachments"),
  text: z27.string().optional().describe("Plain-text attachment content when available"),
  description: z27.string().optional().describe("Caller-provided description of the attachment")
});
var QueryRetrievalModeSchema = z27.enum([
  "graph_only",
  "source_rag",
  "long_context",
  "hybrid"
]);
var SubQuestionSchema = z27.object({
  question: z27.string().describe("Atomic sub-question to retrieve and answer independently"),
  intent: QueryIntentSchema,
  chunkTypes: z27.array(z27.string()).optional().describe("Chunk types to filter retrieval (e.g. coverage, endorsement, declaration)"),
  documentFilters: z27.object({
    type: z27.enum(["policy", "quote"]).optional(),
    carrier: z27.string().optional(),
    insuredName: z27.string().optional(),
    policyNumber: z27.string().optional(),
    quoteNumber: z27.string().optional(),
    linesOfBusiness: z27.array(AcordLobCodeSchema).optional().describe("Filter by ACORD line of business code (e.g. HOME, AUTOP, DISAB) to avoid mixing up similar policies")
  }).optional().describe("Structured filters to narrow document lookup")
});
var QueryClassifyResultSchema = z27.object({
  intent: QueryIntentSchema,
  subQuestions: z27.array(SubQuestionSchema).min(1).describe("Decomposed atomic sub-questions"),
  requiresDocumentLookup: z27.boolean().describe("Whether structured document lookup is needed"),
  requiresChunkSearch: z27.boolean().describe("Whether semantic chunk search is needed"),
  requiresConversationHistory: z27.boolean().describe("Whether conversation history is relevant"),
...
var EvidenceItemSchema = z27.object({
  source: z27.enum(["chunk", "document", "conversation", "attachment", "source_span", "source_node"]),
  chunkId: z27.string().optional(),
  sourceNodeId: z27.string().optional(),
  sourceSpanId: z27.string().optional(),
  documentId: z27.string().optional(),
  turnId: z27.string().optional(),
  attachmentId: z27.string().optional(),
  text: z27.string().describe("Text excerpt from the source"),
  relevance: z27.number().min(0).max(1),
  retrievalMode: QueryRetrievalModeSchema.optional(),
  sourceLocation: SourceSpanLocationSchema.optional(),
  metadata: z27.array(z27.object({ key: z27.string(), value: z27.string() })).optional()
});
var AttachmentInterpretationSchema = z27.object({
  summary: z27.string().describe("Concise summary of what the attachment shows or contains"),
  extractedFacts: z27.array(z27.string()).describe("Specific observable or document facts grounded in the attachment"),
  recommendedFocus: z27.array(z27.string()).describe("Important details to incorporate when answering follow-up questions"),
  confidence: z27.number().min(0).max(1)
});
var RetrievalResultSchema = z27.object({
  subQuestion: z27.string(),
  evidence: z27.array(EvidenceItemSchema)
});
var CitationSchema = z27.object({
  index: z27.number().describe("Citation number [1], [2], etc."),
  chunkId: z27.string().optional().describe("Source chunk ID, e.g. doc-123:coverage:2"),
  sourceNodeId: z27.string().optional().describe("Source tree node ID when available"),
  sourceSpanId: z27.string().optional().describe("Precise source span ID when available"),
  documentId: z27.string(),
  documentType: z27.enum(["policy", "quote"]).optional(),
  field: z27.string().optional().describe("Specific field path, e.g. coverages[0].deductible"),
  quote: z27.string().describe("Exact text from source that supports the claim"),
  relevance: z27.number().min(0).max(1),
  retrievalMode: QueryRetrievalModeSchema.optional(),
  sourceLocation: SourceSpanLocationSchema.optional()
});
var SubAnswerSchema = z27.object({
  subQuestion: z27.string(),
  answer: z27.string(),
  citations: z27.array(CitationSchema),
  confidence: z27.number().min(0).max(1),
  needsMoreContext: z27.boolean().describe("True if evidence was insufficient to answer fully")
});
var VerifyResultSchema = z27.object({
  approved: z27.boolean().describe("Whether all sub-answers are adequately grounded"),
  issues: z27.array(z27.string()).describe("Specific grounding or consistency issues found"),
  retrySubQuestions: z27.array(z27.string()).optional().describe("Sub-questions that need additional retrieval or re-reasoning")
});
var QueryResultSchema = z27.object({
  answer: z27.string(),
  citations: z27.array(CitationSchema),
  intent: QueryIntentSchema,
  confidence: z27.number().min(0).max(1),
  followUp: z27.string().optional().describe("Suggested follow-up question if applicable")
});
...
// src/pce/index.ts
var ReplyAnswersSchema = z28.object({
  answers: z28.array(z28.object({
    questionId: z28.string().optional(),
    fieldPath: z28.string().optional(),
    answer: z28.string()
  }))
});
...
var CarrierInfoSchema = z29.object({
  carrierName: z29.string().describe("Primary insurance company name for display"),
  carrierLegalName: z29.string().optional().describe("Legal entity name of insurer"),
  naicNumber: z29.string().optional().describe("NAIC company code"),
  amBestRating: z29.string().optional().describe("AM Best rating, e.g. 'A+ XV'"),
  admittedStatus: z29.enum(["admitted", "non_admitted", "surplus_lines"]).optional().describe("Admitted status of the carrier"),
  mga: z29.string().optional().describe("Managing General Agent or Program Administrator name"),
  underwriter: z29.string().optional().describe("Named individual underwriter"),
  brokerAgency: z29.string().optional().describe("Broker or producer agency name"),
  brokerContactName: z29.string().optional().describe("Broker or producer contact person name"),
  brokerLicenseNumber: z29.string().optional().describe("Broker or producer license number"),
  policyNumber: z29.string().optional().describe("Policy or quote reference number"),
  effectiveDate: z29.string().optional().describe("Policy effective date (MM/DD/YYYY)"),
  expirationDate: z29.string().optional().describe("Policy expiration date (MM/DD/YYYY)"),
  quoteNumber: z29.string().optional().describe("Quote or proposal reference number"),
  proposedEffectiveDate: z29.string().optional().describe("Proposed effective date for quotes (MM/DD/YYYY)")
});
...
var AdditionalNamedInsuredSchema = z30.object({
  name: z30.string(),
  relationship: z30.string().optional().describe("e.g. subsidiary, affiliate"),
  address: SourceBackedAddressSchema.optional()
}).merge(SourceProvenanceSchema);
var ScheduledPartySchema = z30.object({
  name: z30.string(),
  address: SourceBackedAddressSchema.optional()
}).merge(SourceProvenanceSchema);
var NamedInsuredSchema2 = z30.object({
  insuredName: z30.string().describe("Name of primary named insured"),
  insuredDba: z30.string().optional().describe("Doing-business-as name"),
  insuredAddress: SourceBackedAddressSchema.optional().describe("Primary insured mailing address"),
  insuredEntityType: z30.enum([
    "corporation",
    "llc",
    "partnership",
    "sole_proprietor",
  ...
  insuredFein: z30.string().optional().describe("Federal Employer Identification Number"),
  insuredSicCode: z30.string().optional().describe("SIC code"),
  insuredNaicsCode: z30.string().optional().describe("NAICS code"),
  additionalNamedInsureds: z30.array(AdditionalNamedInsuredSchema).optional().describe("Additional named insureds listed on the policy"),
  lossPayees: z30.array(ScheduledPartySchema).optional().describe("Loss payees listed on the policy"),
  mortgageHolders: z30.array(ScheduledPartySchema).optional().describe("Mortgage holders / lienholders listed on the policy")
});
...
var ExtractorCoverageSchema = CoverageSchema.extend({
  coverageCode: z31.string().optional().describe("Coverage code or class code")
});
var CoverageLimitsSchema = z31.object({
  coverages: z31.array(ExtractorCoverageSchema).describe("All coverages with their limits"),
  coverageForm: z31.enum(["occurrence", "claims_made", "accident"]).optional().describe("Primary coverage trigger type"),
  retroactiveDate: z31.string().optional().describe("Retroactive date for claims-made policies (MM/DD/YYYY)")
});
...
var EndorsementsSchema = z32.object({
  endorsements: z32.array(
    z32.object({
      formNumber: z32.string().describe("Form number, e.g. 'CG 21 47'"),
      editionDate: z32.string().optional().describe("Edition date, e.g. '12 07'"),
      title: z32.string().describe("Endorsement title"),
      endorsementType: z32.enum([
        "additional_insured",
        "waiver_of_subrogation",
        "primary_noncontributory",
        "blanket_additional_insured",
      ...
      effectiveDate: z32.string().optional().describe("Endorsement effective date"),
      affectedCoverageParts: z32.array(z32.string()).optional().describe("Coverage parts affected by this endorsement"),
      namedParties: z32.array(
        z32.object({
          name: z32.string().describe("Party name"),
          role: z32.enum([
            "additional_insured",
            "loss_payee",
            "mortgage_holder",
          ...
          relationship: z32.string().optional().describe("Relationship to insured"),
          scope: z32.string().optional().describe("Scope of coverage for this party")
        })
      ).optional().describe("Named parties (additional insureds, loss payees, etc.)"),
      keyTerms: z32.array(z32.string()).optional().describe("Key terms or notable provisions in the endorsement"),
      premiumImpact: z32.string().optional().describe("Additional premium or credit"),
      excerpt: z32.string().optional().describe("Short source excerpt, not full verbatim text"),
      content: z32.string().optional().describe("Legacy fallback only; do not return full text when sourceSpanIds are available"),
      pageStart: z32.number().describe("Starting page number of this endorsement"),
      pageEnd: z32.number().optional().describe("Ending page number of this endorsement"),
      sourceSpanIds: z32.array(z32.string()).optional().describe("Source span IDs grounding this endorsement"),
      sourceTextHash: z32.string().optional().describe("Hash of the source text when available")
    })
...
var ExclusionsSchema = z33.object({
  exclusions: z33.array(
    z33.object({
      name: z33.string().describe("Exclusion title or short description"),
      formNumber: z33.string().optional().describe("Form number if part of a named endorsement"),
      excludedPerils: z33.array(z33.string()).optional().describe("Specific perils excluded"),
      isAbsolute: z33.boolean().optional().describe("Whether the exclusion is absolute (no exceptions)"),
      exceptions: z33.array(z33.string()).optional().describe("Exceptions to the exclusion, if any"),
      buybackAvailable: z33.boolean().optional().describe("Whether coverage can be bought back via endorsement"),
      buybackEndorsement: z33.string().optional().describe("Form number of the buyback endorsement if available"),
      appliesTo: z33.array(z33.string()).optional().describe("Policy types this exclusion applies to"),
      content: z33.string().describe("Full verbatim exclusion text"),
      pageNumber: z33.number().optional().describe("Page number where exclusion appears")
    })
...
var ConditionsSchema = z34.object({
  conditions: z34.array(
    z34.object({
      name: z34.string().describe("Condition title"),
      conditionType: z34.enum([
        "duties_after_loss",
        "notice_requirements",
        "other_insurance",
        "cancellation",
      ...
      content: z34.string().describe("Full verbatim condition text"),
      keyValues: z34.array(
        z34.object({
          key: z34.string().describe("Key name (e.g. 'noticePeriod', 'suitDeadline')"),
          value: z34.string().describe("Value (e.g. '30 days', '2 years')")
        })
      ).optional().describe("Key values extracted from the condition (notice periods, deadlines, etc.)"),
      pageNumber: z34.number().optional().describe("Page number where condition appears")
    })
...
var PremiumBreakdownSchema = z35.object({
  premium: z35.string().optional().describe("Total premium amount, e.g. '$5,000'"),
  premiumAmount: z35.number().optional().describe("Total premium as a plain number with no currency symbols or commas"),
  totalCost: z35.string().optional().describe("Total cost including taxes and fees, e.g. '$5,250'"),
  totalCostAmount: z35.number().optional().describe("Total cost as a plain number with no currency symbols or commas"),
  premiumBreakdown: z35.array(
    z35.object({
      line: z35.string().describe("Coverage line name"),
      amount: z35.string().describe("Premium amount for this line"),
      amountValue: z35.number().optional().describe("Premium amount as a plain number with no currency symbols or commas")
    })
  ).optional().describe("Per-coverage-line premium breakdown"),
  taxesAndFees: z35.array(
    z35.object({
      name: z35.string().describe("Fee or tax name"),
      amount: z35.string().describe("Dollar amount"),
      amountValue: z35.number().optional().describe("Fee or tax amount as a plain number with no currency symbols or commas"),
      type: z35.enum(["tax", "fee", "surcharge", "assessment"]).optional().describe("Fee category")
    })
  ).optional().describe("Taxes, fees, surcharges, and assessments"),
  minimumPremium: z35.string().optional().describe("Minimum premium if stated"),
  minimumPremiumAmount: z35.number().optional().describe("Minimum premium as a plain number when the source states a fixed amount"),
  depositPremium: z35.string().optional().describe("Deposit premium if stated"),
  depositPremiumAmount: z35.number().optional().describe("Deposit premium as a plain number when the source states a fixed amount"),
  paymentPlan: z35.string().optional().describe("Payment plan description"),
  auditType: z35.enum(["annual", "semi_annual", "quarterly", "monthly", "final", "self"]).optional().describe("Premium audit type"),
  ratingBasis: z35.string().optional().describe("Rating basis, e.g. payroll, revenue, area, units")
});
...
var DeclarationsFieldSchema = z36.object({
  field: z36.string().describe("Descriptive field name (e.g. 'policyNumber', 'effectiveDate', 'coverageALimit')"),
  value: z36.string().describe("Extracted value exactly as it appears in the document"),
  section: z36.string().optional().describe("Section or grouping this field belongs to (e.g. 'Coverage Limits', 'Vehicle Schedule')")
});
var DeclarationsExtractSchema = z36.object({
  fields: z36.array(DeclarationsFieldSchema).describe("All declarations page fields extracted as key-value pairs. Structure varies by line of business.")
});
...
var LossHistorySchema = z37.object({
  lossSummary: z37.string().optional().describe("Summary of loss history, e.g. '3 claims in past 5 years totaling $125,000'"),
  individualClaims: z37.array(
    z37.object({
      date: z37.string().optional().describe("Date of loss or claim"),
      type: z37.string().optional().describe("Type of claim, e.g. 'property damage', 'bodily injury'"),
      description: z37.string().optional().describe("Brief description of the claim"),
      amountPaid: z37.string().optional().describe("Amount paid"),
      amountReserved: z37.string().optional().describe("Amount reserved"),
      status: z37.enum(["open", "closed", "reopened"]).optional().describe("Claim status"),
      claimNumber: z37.string().optional().describe("Claim reference number")
    })
  ).optional().describe("Individual claim records"),
  experienceMod: z37.string().optional().describe("Experience modification factor for workers comp, e.g. '0.85'")
});
...
var SubsectionSchema2 = z38.object({
  title: z38.string().describe("Subsection title"),
  sectionNumber: z38.string().optional().describe("Subsection number"),
  pageNumber: z38.number().optional().describe("Page number"),
  excerpt: z38.string().optional().describe("Short source excerpt, not full verbatim text"),
  content: z38.string().optional().describe("Legacy fallback only; do not return full text when sourceSpanIds are available"),
  sourceSpanIds: z38.array(z38.string()).optional().describe("Source span IDs grounding this subsection"),
  sourceTextHash: z38.string().optional().describe("Hash of the source text when available")
});
var SectionsSchema = z38.object({
  sections: z38.array(
    z38.object({
      title: z38.string().describe("Section title"),
      type: z38.enum([
        "declarations",
        "insuring_agreement",
        "policy_form",
      ...
      excerpt: z38.string().optional().describe("Short source excerpt, not full verbatim text"),
      content: z38.string().optional().describe("Legacy fallback only; do not return full text when sourceSpanIds are available"),
      pageStart: z38.number().describe("Starting page number"),
      pageEnd: z38.number().optional().describe("Ending page number"),
      sourceSpanIds: z38.array(z38.string()).optional().describe("Source span IDs grounding this section"),
      sourceTextHash: z38.string().optional().describe("Hash of the source text when available"),
      subsections: z38.array(SubsectionSchema2).optional().describe("Subsections within this section")
    })
...
var AuxiliaryFactSchema2 = z39.object({
  key: z39.string().describe("Normalized machine-readable fact key, e.g. 'policyholder_age' or 'insured_name'"),
  value: z39.string().describe("Concrete extracted fact value"),
  subject: z39.string().optional().describe("Person, entity, vehicle, property, or schedule item this fact belongs to"),
  context: z39.string().optional().describe("Short disambiguating context, such as 'Driver Schedule' or 'Named Insured'")
});
var SupplementarySchema = z39.object({
  regulatoryContacts: z39.array(ContactSchema).optional().describe("Regulatory body contacts (state department of insurance, ombudsman)"),
  claimsContacts: z39.array(ContactSchema).optional().describe("Claims reporting contacts and instructions"),
  thirdPartyAdministrators: z39.array(ContactSchema).optional().describe("Third-party administrators for claims handling"),
  cancellationNoticeDays: z39.number().optional().describe("Required notice period for cancellation in days"),
  nonrenewalNoticeDays: z39.number().optional().describe("Required notice period for nonrenewal in days"),
  auxiliaryFacts: z39.array(AuxiliaryFactSchema2).optional().describe("Additional retrieval-only facts that do not fit the strict primary schema")
});
...
var DefinitionsSchema = z40.object({
  definitions: z40.array(
    z40.object({
      term: z40.string().describe("Defined term exactly as shown in the document"),
      definition: z40.string().describe("Full verbatim definition text, preserving original wording"),
      pageNumber: z40.number().optional().describe("Original document page number"),
      formNumber: z40.string().optional().describe("Form number where this definition appears"),
      formTitle: z40.string().optional().describe("Form title where this definition appears"),
      sectionRef: z40.string().optional().describe("Definition section heading or subsection reference"),
      originalContent: z40.string().optional().describe("Short verbatim source snippet containing the term and definition")
    })
...
var CoveredReasonsSchema = z41.object({
  coveredReasons: z41.array(
    z41.object({
      coverageName: z41.string().describe("Coverage, coverage part, or form this covered reason belongs to"),
      reasonNumber: z41.string().optional().describe("Source number or letter for the covered reason, if shown"),
      title: z41.string().optional().describe("Covered reason title, peril, cause of loss, trigger, or short name"),
      content: z41.string().describe("Full verbatim covered-reason or insuring-agreement text"),
      conditions: z41.array(z41.string()).optional().describe("Conditions, timing rules, documentation requirements, or prerequisites attached to this covered reason"),
      exceptions: z41.array(z41.string()).optional().describe("Exceptions or limitations attached to this covered reason"),
      appliesTo: z41.array(z41.string()).optional().describe("Covered property, persons, autos, locations, operations, or coverage parts this reason applies to"),
      pageNumber: z41.number().optional().describe("Original document page number"),
      formNumber: z41.string().optional().describe("Form number where this covered reason appears"),
      formTitle: z41.string().optional().describe("Form title where this covered reason appears"),
      sectionRef: z41.string().optional().describe("Section heading where this covered reason appears"),
      originalContent: z41.string().optional().describe("Short verbatim source snippet used for this covered reason")
    })
...
function deriveCoveredReasonsFromSections(data) {
  const coveredReasons = getSections(data).filter(sectionLooksLikeCoveredReason).map((section) => ({
...
function deriveDefinitionsFromSections(data) {
  const definitions = getSections(data).filter((section) => String(section.type ?? "").toLowerCase() === "definition").map((section) => ({
...
// src/prompts/templates/personal-auto.ts
var PERSONAL_AUTO_TEMPLATE = {
  type: "personal_auto",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
// src/prompts/templates/general-liability.ts
var GENERAL_LIABILITY_TEMPLATE = {
...
// src/prompts/templates/commercial-property.ts
var COMMERCIAL_PROPERTY_TEMPLATE = {
...
// src/prompts/templates/commercial-auto.ts
var COMMERCIAL_AUTO_TEMPLATE = {
...
// src/prompts/templates/workers-comp.ts
var WORKERS_COMP_TEMPLATE = {
  type: "workers_comp",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
// src/prompts/templates/umbrella-excess.ts
var UMBRELLA_EXCESS_TEMPLATE = {
...
// src/prompts/templates/professional-liability.ts
var PROFESSIONAL_LIABILITY_TEMPLATE = {
...
// src/prompts/templates/cyber.ts
var CYBER_TEMPLATE = {
  type: "cyber",
  expectedSections: [
    "carrier_info",
...
// src/prompts/templates/directors-officers.ts
var DIRECTORS_OFFICERS_TEMPLATE = {
...
// src/prompts/templates/crime.ts
var CRIME_TEMPLATE = {
  type: "crime_fidelity",
  expectedSections: [
    "carrier_info",
...
// src/prompts/templates/dwelling-fire.ts
var DWELLING_FIRE_TEMPLATE = {
  type: "dwelling_fire",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
// src/prompts/templates/flood.ts
var FLOOD_TEMPLATE = {
  type: "flood",
  expectedSections: [
    "carrier_info",
...
// src/prompts/templates/earthquake.ts
var EARTHQUAKE_TEMPLATE = {
  type: "earthquake",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
// src/prompts/templates/personal-umbrella.ts
var PERSONAL_UMBRELLA_TEMPLATE = {
...
// src/prompts/templates/personal-articles.ts
var PERSONAL_ARTICLES_TEMPLATE = {
...
// src/prompts/templates/watercraft.ts
var WATERCRAFT_TEMPLATE = {
  type: "watercraft",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
// src/prompts/templates/recreational-vehicle.ts
var RECREATIONAL_VEHICLE_TEMPLATE = {
...
// src/prompts/templates/farm-ranch.ts
var FARM_RANCH_TEMPLATE = {
  type: "farm_ranch",
  expectedSections: [
    "carrier_info",
    "named_insured",
...
function getTemplate(policyType) {
...
export {
  ...
  EndorsementPartySchema,
  EndorsementSchema,
  EndorsementTypeSchema,
  EnrichedCoverageSchema,
