...
import { clRouterMessagesHaveVision, mapSpotCallToClRouterPrimitive, } from "./clRouterPrimitive.js";
...
export class ClRouterHttpError extends Error {
    ...
    constructor(status, statusText, options) {
        ...
        this.status = status;
        if (options?.routerCode !== undefined)
            this.routerCode = options.routerCode;
...
export function normalizeClRouterTrace(trace) {
    if (!trace)
        return undefined;
    const normalized = {};
    const tags = {};
    for (const [key, value] of Object.entries(trace)) {
        ...
        if ((key === "traceId" || key === "parentRequestId" || key === "caller") &&
            typeof value === "string") {
...
export function wireClRouterGenerateRequest(request) {
    const body = {
...
const CL_ROUTER_FAILURE_CODE_SET = new Set(CL_ROUTER_FAILURE_CODES);
function isClRouterFailureCode(value) {
...
function assertFetchableAssetUrl(rawUrl) {
    ...
    if (url.protocol !== "https:" && !(url.protocol === "http:" && isLoopback)) {
        throw new ClRouterProtocolError("Referenced router assets require HTTPS URLs unless they target loopback");
    }
    return url.toString();
}
function messageAssetSizes(request) {
    return (request.messages ?? []).flatMap((message) => message.content.flatMap((part) => {
        if (part.type === "text")
            return [];
        if ("source" in part)
            return [part.source.sizeBytes];
        const data = part.type === "image" ? part.image : part.data;
        return [Buffer.from(data, "base64").byteLength];
    }));
}
function assertRequestLimits(request) {
    const assetSizes = messageAssetSizes(request);
    if (assetSizes.length > CL_ROUTER_MAX_ASSETS) {
        throw new ClRouterProtocolError(`cl-router accepts at most ${CL_ROUTER_MAX_ASSETS} assets per request`);
    }
    if (assetSizes.some((size) => size <= 0 || size > CL_ROUTER_MAX_ASSET_BYTES)) {
        throw new ClRouterProtocolError(`cl-router assets must be between 1 and ${CL_ROUTER_MAX_ASSET_BYTES} bytes`);
    }
    const aggregateBytes = assetSizes.reduce((total, size) => total + size, 0);
    if (aggregateBytes > CL_ROUTER_MAX_AGGREGATE_ASSET_BYTES) {
        throw new ClRouterProtocolError(`cl-router assets exceed the ${CL_ROUTER_MAX_AGGREGATE_ASSET_BYTES}-byte aggregate limit`);
    }
    if (Buffer.byteLength(JSON.stringify(request)) > CL_ROUTER_MAX_JSON_BYTES) {
        throw new ClRouterProtocolError(`cl-router request exceeds the ${CL_ROUTER_MAX_JSON_BYTES}-byte JSON limit`);
    }
}
...
function assertRouterCanFetchAssets(request, routerUrl) {
    if (isLoopbackUrl(routerUrl))
        return;
    for (const message of request.messages ?? []) {
        for (const part of message.content) {
            if (part.type === "text" || !("source" in part))
                continue;
            if (isLoopbackUrl(new URL(part.source.url))) {
                throw new ClRouterProtocolError("Loopback router assets require a loopback cl-router deployment");
            }
        }
    }
}
function requestInput(prompt, assets) {
    const images = [];
    for (const image of assets?.images ?? []) {
        ...
        if (!imageBase64) {
            throw new ClRouterProtocolError("Router image input must contain nonempty base64 data");
        }
            ...
            messages: [
                {
                    role: "user",
                    content: [
                        ...images.map((image) => "source" in image
...
export function createClRouterClient(options) {
    const baseUrl = new URL(options.baseUrl);
    const isLoopback = isLoopbackUrl(baseUrl);
    if (baseUrl.protocol !== "https:" &&
        !(baseUrl.protocol === "http:" && isLoopback)) {
        throw new Error("CL_ROUTER_URL must use HTTPS unless it targets loopback localhost, 127.0.0.1, or ::1");
    }
    if (!options.secret.trim())
        throw new Error("CL_ROUTER_SECRET is required");
    ...
    return {
        async generate(input, executeJob) {
            ...
            if (executeJob) {
                return parseGenerateResponse(await executeJob(body));
            }
            const generateUrl = new URL(request.route ? "v1/manual" : "v1/generate", origin);
            let response;
            try {
            ...
            try {
                ...
                throw new ClRouterProtocolError(`cl-router returned non-JSON success response: ${error instanceof Error ? error.name : "unknown"}`);
            }
            return parseGenerateResponse(payload);
        },
    };
}
//# sourceMappingURL=clRouterClient.js.map