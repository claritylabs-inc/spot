...
function isExtractionFamily(task, taskKind) {
    return ((task?.startsWith("extraction") ?? false) ||
...
function isReasoningFamily(task, taskKind) {
    if (isExtractionFamily(task, taskKind))
        return true;
    if (task && REASONING_TASKS.has(task))
        return true;
    if (taskKind?.startsWith("pce_"))
        return true;
    return (taskKind === "query_reason" ||
        taskKind === "query_plan" ||
        taskKind === "query_verify" ||
        taskKind === "carrier_identity_selection");
}
function isAgentToolTurn(task, taskKind) {
    return ((task !== undefined && AGENT_TOOL_TASKS.has(task)) ||
        (taskKind !== undefined && AGENT_TOOL_TASK_KINDS.has(taskKind)));
}
function compactRequirements(flags) {
    const requirements = {};
    if (flags.vision)
...
export function mapSpotCallToClRouterPrimitive(input) {
    ...
    if ((task !== undefined && TEXT_TASKS.has(task)) ||
...
export function clRouterContentHasVision(content) {
...
export function clRouterMessagesHaveVision(messages) {
    return (messages ?? []).some((message) => clRouterContentHasVision(message.content));
}
//# sourceMappingURL=clRouterPrimitive.js.map