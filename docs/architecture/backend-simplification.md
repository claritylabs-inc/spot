...
The shared-development `brokerActivity` inventory contains 38 retired status notifications (37 extraction completions and one upload), with source policy references rather than unique policy facts. The backed-up retired-store purge removes these unused duplicates; final inventory counts are recorded again after migration. Production has zero rows in this table.
