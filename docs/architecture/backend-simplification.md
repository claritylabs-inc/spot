...
| Company research | Verify a successful router-backed creation without a website, including supported classification and cited wiki contributions. Fix evidenced identity/concurrency/source defects. | Live isolated-fixture outcome, manual-prose preservation, and focused regressions. |
| Browser workflows | Native operator/client OAuth consent, token rotation/revocation, compliance source Notes persistence, and company-wiki import passed in headed local Chrome. | [Workflow ledger](../testing/workflow-qa.md) records the actors, assertions, and cleanup. Request-editor replacement must still preserve failed drafts before release. |
