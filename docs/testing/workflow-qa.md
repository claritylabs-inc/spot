...
| WEBMCP | Passed scripted headless run (September 24, 2026), with model steps blocked | 143/146 checks passed; the other 3 were blocked because the local router rejected every job with 422: the deeper requirement check, requirement import, and agent email drafting. Artifact: `.context/qa/webmcp/results.json` plus screenshots. The run executed 91 of the 126 imperative tools and all 7 declarative tools against native-local Convex, with email in capture mode. Covered: signup, member vs admin scoping, compliance/connect/settings/team/profile/thread writes, draft send on synthetic local drafts, seeded certificate reissue/edit/archive/restore, and public token pages. Not executed: Slack, IMAP mailbox and mailbox-review tools (no Slack worker or mailbox), and destructive profile deletion. It used a stub `modelContext`, not Chrome's native WebMCP. |
